...
window.DashCore = (function () {
  ...
  function renderTable(tbodyId, rows, cols) {
    ...
    tb.innerHTML = rows.map((r) =>
      `<tr>${cols.map((c) => {
        ...
        if (c.fmt === "raw") v = v ?? "—";
        if (c.cls && r[c.cls]) v = `<span class="tag-${r[c.cls]}">${v}</span>`;
  ...
  function bindChartNav(chart, opts) {
    ...
    chart.on("click", (params) => {
      ...
      const patch = {};
      if (nav.brand) patch.brand = nav.brand;
  ...
  function setBarChart(chart, x, series, opts) {
    if (!chart) return;
    const barMaxWidth = opts?.barMaxWidth ?? 36;
    chart.setOption({
      color: THEME.palette,
      tooltip: { trigger: "axis" },
      ...
      grid: { left: 48, right: 16, bottom: 32, top: 40, containLabel: true },
      xAxis: {
        type: opts?.horizontal ? "value" : "category",
        data: opts?.horizontal ? undefined : x,
        axisLabel: { color: THEME.muted },
      },
      yAxis: {
        type: opts?.horizontal ? "category" : "value",
        data: opts?.horizontal ? x : undefined,
        axisLabel: { color: THEME.muted },
      },
      series: series.map((s) => ({ name: s.name, type: "bar", data: s.data, barMaxWidth })),
    });
  }
  ...
  function setScatter(chart, rows, xKey, yKey, nameKey) {
    if (!chart || !rows?.length) return;
    chart.setOption({
      color: [THEME.accent],
      tooltip: {
        formatter: (p) => `${p.data[2]}<br/>${xKey}: ${fmtNum(p.data[0])}<br/>${yKey}: ${fmtNum(p.data[1])}`,
      },
      grid: { left: 48, right: 16, bottom: 32, top: 24, containLabel: true },
      xAxis: { name: xKey, axisLabel: { color: THEME.muted } },
      yAxis: { name: yKey, axisLabel: { color: THEME.muted } },
      series: [{
        type: "scatter",
        symbolSize: 12,
        data: rows.map((r) => [r[xKey], r[yKey], r[nameKey]]),
      }],
    });
  }

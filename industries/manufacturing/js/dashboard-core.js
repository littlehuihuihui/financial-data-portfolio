...
window.DashCore = (function () {
  ...
  const THEME = {
    accent: "#e67e22",
    palette: ["#e67e22", "#d35400", "#f39c12", "#27ae60", "#2980b9", "#7f8c8d"],
    muted: "#8b95a8",
    text: "#e8edf5",
  };
  const charts = {};
  let DATA_SOURCE = "manufacturing_analytics · MySQL ADS/DWS";
  let usingDemo = false;
  ...
  async function api(path, params) {
    ...
    try {
      ...
      usingDemo = false;
      DATA_SOURCE = "manufacturing_analytics · MySQL ADS/DWS";
  ...
  function renderTable(tbodyId, rows, cols) {
    ...
    tb.innerHTML = rows.map((r) =>
      `<tr>${cols.map((c) => {
        ...
        if (c.fmt === "tag" || c.key === "alert_level") {
          const lvl = v ?? "—";
          v = `<span class="tag-${lvl}">${lvl}</span>`;
        }
  ...
  function bindChartNav(chart, opts) {
    ...
    chart.on("click", (params) => {
      ...
      const patch = {};
      if (nav.factory) patch.factory = nav.factory;

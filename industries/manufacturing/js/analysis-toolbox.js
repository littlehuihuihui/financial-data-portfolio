      ...
      <article class="toolbox-card" id="toolbox-${esc(item.id)}">
        ...
        <header class="toolbox-card-head">
          ...
          <dl class="toolbox-meta">
            ...
            <div class="toolbox-meta-block"><dt>解释：</dt><dd>${esc(item.explain || item.what)}</dd></div>

            <div class="toolbox-meta-block"><dt>业务问题：</dt><dd>${esc(item.businessQuestion || "—")}</dd></div>

            <div class="toolbox-meta-block"><dt>作品集应用示例：</dt><dd>${esc(item.portfolio)}</dd></div>
  ...
  function initChart(item, idx) {
    ...
    if (type === "pareto") {
      ...
      ch.setOption({
        ...
        legend: { data: ["收入(万)", "累计%"], textStyle: { color: "#94a3b8" } },

        xAxis: { type: "category", data: d.labels, axisLabel: { color: "#94a3b8" } },

        yAxis: [{ type: "value", name: "万" }, { type: "value", name: "%", max: 100 }],

        series: [

          { name: "收入(万)", type: "bar", data: d.values, itemStyle: { color: "#3b82f6" } },

          { name: "累计%", type: "line", yAxisIndex: 1, data: d.cumulative, itemStyle: { color: "#f472b6" } },


/* =========================================================
 * 数据知识图谱 · DATA NEXUS（单文件 · 硬编码 · D3 流水线布局）
 * ========================================================= */
    const CATEGORIES = {
      storage: { label: "STORAGE 存储", color: "#4da3ff" },
      process: { label: "PROCESS 加工", color: "#f59e0b" },
      analyze: { label: "ANALYZE 分析", color: "#34d399" },
      govern:  { label: "GOVERN 治理",  color: "#a78bfa" },
      intel:   { label: "INTEL 智能",   color: "#fb7185" }
    };

    const COMPARE_FIELDS = [
      { key: "type", label: "类型" },
      { key: "workload", label: "负载" },
      { key: "storage", label: "存储" },
      { key: "language", label: "语言/语法" },
      { key: "data", label: "典型数据" },
      { key: "pros", label: "优点" },
      { key: "cons", label: "缺点" },
      { key: "scenario", label: "典型场景" }
    ];

    const STAGES = [
      { key: "src", label: "01 源头" },
      { key: "lang", label: "02 语言" },
      { key: "proc", label: "03 加工" },
      { key: "store", label: "04 存储" },
      { key: "ana", label: "05 分析" },
      { key: "ai", label: "06 智能" }
    ];

    const nodes = [
  {
    "id": "database",
    "name": "数据库",
    "category": "storage",
    "catalog": true,
    "catalogLabel": "引擎列表",
    "detail": {
      "subtitle": "事务与分析的持久化系统 · 多引擎选型目录",
      "l1": {
        "definition": "数据库按数据模型组织存储并提供查询/事务能力：关系、文档、键值、列式、向量、搜索等。",
        "why": "是业务真相来源，也是 ETL/实时/仓的上游；选错引擎会把成本与性能问题带进整条链路。",
        "source": "《数据库系统概念》（机械工业出版社）；《智能数据工程》（清华大学出版社）"
      },
      "l2": {
        "principles": [
          "先分负载：OLTP vs OLAP vs HTAP",
          "模型决定语法与存法",
          "可靠三件套：WAL/复制、索引计划、备份恢复"
        ],
        "terms": [
          "OLTP",
          "OLAP",
          "ACID",
          "WAL",
          "MVCC",
          "列式存储"
        ],
        "topics": [
          {
            "name": "OLTP vs OLAP",
            "text": "OLTP：少量行、事务、低延迟（PG/MySQL）。OLAP：大批量扫描聚合（ClickHouse/仓）。"
          },
          {
            "name": "多引擎组合",
            "text": "主库 + 缓存 Redis + 分析 CH + 搜索 ES + 向量 Milvus/pgvector 是常态。"
          },
          {
            "name": "怎么选型",
            "text": "强一致交易→PG/MySQL；灵活文档→Mongo；热点→Redis；海量聚合→CH；语义检索→向量；全文→ES。"
          }
        ],
        "deps": {
          "upstream": [],
          "downstream": [
            "SQL",
            "ETL",
            "实时数据",
            "数据仓库",
            "数据建模"
          ]
        },
        "source": "各引擎官方文档；机械工业出版社《数据库系统概念》"
      },
      "l3": {
        "tools": [
          "PostgreSQL",
          "MySQL",
          "MongoDB",
          "Redis",
          "ClickHouse",
          "Milvus",
          "pgvector",
          "Elasticsearch"
        ],
        "code": "-- 见各引擎 L3 示例；先选型再深挖语法",
        "resources": [
          {
            "name": "《数据库系统概念》",
            "note": "机械工业出版社"
          },
          {
            "name": "各引擎官方文档",
            "note": "PG/MySQL/Redis/CH/..."
          }
        ],
        "source": "官方文档 + 教材"
      }
    },
    "engines": [
      {
        "id": "postgresql",
        "name": "PostgreSQL",
        "tagline": "开放标准关系库 · OLTP/扩展强",
        "compare": {
          "type": "关系型 RDBMS",
          "workload": "OLTP（亦可轻度分析）",
          "storage": "行存为主；磁盘 + WAL",
          "language": "标准 SQL + PL/pgSQL；扩展丰富",
          "data": "业务表、JSONB、地理、向量(pgvector)",
          "pros": "标准合规、约束严谨、扩展生态（PostGIS/pgvector）",
          "cons": "超大规模分析不如专用列存；运维需掌握 vacuum",
          "scenario": "核心业务库、复杂查询、地理/向量入门"
        },
        "detail": {
          "subtitle": "对象关系型数据库 · 扩展生态之王",
          "l1": {
            "definition": "PostgreSQL 是功能完备的开源对象关系型数据库，强调 SQL 标准与可扩展性。",
            "why": "适合作为「真相来源」主库：事务、约束、复杂查询与 JSONB/地理/向量扩展可同库演进。",
            "source": "PostgreSQL 官方文档；《数据库系统概念》"
          },
          "l2": {
            "principles": [
              "MVCC 实现高并发读写",
              "WAL 保证崩溃恢复与复制",
              "优化器基于代价选择计划",
              "扩展机制把能力塞进同一引擎"
            ],
            "terms": [
              "MVCC",
              "WAL / PITR",
              "JSONB",
              "扩展 Extensions",
              "EXPLAIN ANALYZE"
            ],
            "topics": [
              {
                "name": "存什么",
                "text": "结构化业务表为主；JSONB 半结构化；PostGIS 地理；pgvector 向量。"
              },
              {
                "name": "语法特点",
                "text": "高度贴近标准 SQL；窗口函数、CTE、UPSERT（ON CONFLICT）成熟。",
                "code": "BEGIN;\nUPDATE accounts SET balance=balance-100 WHERE id=1;\nUPDATE accounts SET balance=balance+100 WHERE id=2;\nCOMMIT;"
              },
              {
                "name": "快在哪 / 适用",
                "text": "复杂查询、约束完整性、扩展能力；中大型 OLTP。",
                "children": [
                  {
                    "name": "不适用",
                    "text": "百亿行即席 OLAP 宽表扫描——应同步到 ClickHouse/数仓。"
                  }
                ]
              },
              {
                "name": "运维要点",
                "text": "autovacuum、连接池（PgBouncer）、主从/Patroni、备份 PITR。"
              }
            ],
            "deps": {
              "upstream": [],
              "downstream": [
                "SQL",
                "ETL",
                "数据建模"
              ]
            },
            "source": "https://www.postgresql.org/docs/"
          },
          "l3": {
            "tools": [
              "psql",
              "PgBouncer",
              "PostGIS",
              "pgvector"
            ],
            "code": "CREATE INDEX idx_orders_user_dt ON orders(user_id, created_at);\nEXPLAIN ANALYZE SELECT * FROM orders WHERE user_id=88;",
            "resources": [
              {
                "name": "PostgreSQL Docs",
                "note": "官方文档"
              },
              {
                "name": "《PostgreSQL 修炼之道》",
                "note": "运维与优化"
              }
            ],
            "source": "PostgreSQL Documentation"
          }
        }
      },
      {
        "id": "mysql",
        "name": "MySQL",
        "tagline": "互联网 OLTP 主力 · InnoDB",
        "compare": {
          "type": "关系型 RDBMS",
          "workload": "OLTP 高并发",
          "storage": "InnoDB 行存 + redo/undo",
          "language": "SQL（方言：LIMIT、部分函数差异）",
          "data": "业务二维表、部分 JSON",
          "pros": "生态成熟、运维资料多、主从与分库分表方案多",
          "cons": "复杂分析/窗口函数历史弱于 PG；大表 DDL 需谨慎",
          "scenario": "Web/App 主库、订单账户、高并发读写"
        },
        "detail": {
          "subtitle": "开源关系库 · Web 时代默认选项",
          "l1": {
            "definition": "MySQL（InnoDB）是广泛使用的开源关系型数据库，擅长高并发短事务。",
            "why": "互联网业务存量最大；招聘与运维经验丰富，分库分表生态完善。",
            "source": "MySQL 官方文档；《高性能 MySQL》"
          },
          "l2": {
            "principles": [
              "InnoDB 聚簇索引组织表",
              "主从复制扩展读",
              "用分库分表水平扩展写"
            ],
            "terms": [
              "InnoDB",
              "Binlog",
              "聚簇索引",
              "MVCC",
              "半同步复制"
            ],
            "topics": [
              {
                "name": "存什么",
                "text": "业务表（用户/订单/库存）；避免当分析仓。"
              },
              {
                "name": "语法与差异",
                "text": "LIMIT 分页；部分函数名与 PG 不同；JSON 支持逐渐增强。",
                "code": "UPDATE stock SET qty=qty-1 WHERE sku='A' AND qty>=1;\nSELECT * FROM orders WHERE user_id=88 ORDER BY id DESC LIMIT 20;"
              },
              {
                "name": "适用 / 不适用",
                "text": "适用：高并发 OLTP。不适用：重分析、复杂递归 CTE 场景可评估 PG。"
              },
              {
                "name": "优化要点",
                "text": "最左前缀、覆盖索引、避免回表；大事务拆分；pt-osc 做在线 DDL。"
              }
            ],
            "deps": {
              "upstream": [],
              "downstream": [
                "SQL",
                "ETL",
                "实时数据"
              ]
            },
            "source": "https://dev.mysql.com/doc/"
          },
          "l3": {
            "tools": [
              "mysql CLI",
              "ProxySQL",
              "Vitess / ShardingSphere"
            ],
            "code": "EXPLAIN SELECT * FROM orders WHERE user_id=88 AND created_at>='2024-01-01';",
            "resources": [
              {
                "name": "《高性能 MySQL》",
                "note": "电子工业出版社"
              },
              {
                "name": "MySQL Reference Manual",
                "note": "官方"
              }
            ],
            "source": "MySQL Reference Manual"
          }
        }
      },
      {
        "id": "mongodb",
        "name": "MongoDB",
        "tagline": "文档型 · 灵活 Schema",
        "compare": {
          "type": "文档型 NoSQL",
          "workload": "灵活读写 / 中等事务",
          "storage": "BSON 文档；WiredTiger",
          "language": "MQL（find/aggregate），非 SQL",
          "data": "嵌套 JSON 文档、数组、半结构化",
          "pros": "Schema 灵活、水平分片、聚合管道强",
          "cons": "多文档强一致关联弱于 RDBMS；易堆出失控文档",
          "scenario": "内容中台、画像草稿、快速迭代业务对象"
        },
        "detail": {
          "subtitle": "文档数据库 · BSON 为中心",
          "l1": {
            "definition": "MongoDB 以 BSON 文档存储数据，用查询语言 MQL 与聚合管道操作。",
            "why": "字段多变、嵌套结构多的业务用关系表会很痛苦时，文档模型更自然。",
            "source": "MongoDB Manual"
          },
          "l2": {
            "principles": [
              "面向文档而非范式表",
              "聚合管道 = 声明式变换流",
              "副本集高可用 + 分片扩展"
            ],
            "terms": [
              "BSON",
              "Replica Set",
              "Shard Key",
              "Aggregation Pipeline",
              "事务（多文档有限）"
            ],
            "topics": [
              {
                "name": "存什么",
                "text": "一篇文档可嵌订单行、标签数组；适合内容/配置/事件型对象。"
              },
              {
                "name": "语法",
                "text": "db.col.find / updateOne / aggregate；不是 SQL。",
                "code": "db.orders.find({status:\"paid\",amount:{$gte:100}})\ndb.orders.aggregate([\n  {$match:{status:\"paid\"}},\n  {$group:{_id:\"$region\",gmv:{$sum:\"$amount\"}}}\n])"
              },
              {
                "name": "适用 / 不适用",
                "text": "适用：灵活文档。不适用：强范式多表事务（账务核心仍建议 RDBMS）。"
              }
            ],
            "deps": {
              "upstream": [],
              "downstream": [
                "ETL",
                "Python"
              ]
            },
            "source": "https://www.mongodb.com/docs/"
          },
          "l3": {
            "tools": [
              "mongosh",
              "Compass",
              "Atlas"
            ],
            "code": "// update\ndb.users.updateOne({_id:88},{$set:{vip:true}})",
            "resources": [
              {
                "name": "MongoDB Manual",
                "note": "官方"
              },
              {
                "name": "Aggregation 教程",
                "note": "官方"
              }
            ],
            "source": "MongoDB Documentation"
          }
        }
      },
      {
        "id": "redis",
        "name": "Redis",
        "tagline": "内存结构 · 微秒级缓存/队列",
        "compare": {
          "type": "内存键值 / 结构服务",
          "workload": "超低延迟缓存与协作",
          "storage": "内存为主；RDB/AOF 可选持久化",
          "language": "Redis 命令协议 / 客户端 SDK",
          "data": "String/Hash/List/Set/ZSet/Stream",
          "pros": "极快、结构丰富、适合锁/限流/排行榜",
          "cons": "内存成本高；不能当唯一真相库；大数据集需拆分",
          "scenario": "Session、热点缓存、分布式锁、排行榜、轻量队列"
        },
        "detail": {
          "subtitle": "内存数据结构服务器",
          "l1": {
            "definition": "Redis 将丰富数据结构放在内存中提供微秒～毫秒级访问，并可配置持久化。",
            "why": "它是速度层而非账本：真相仍在 PG/MySQL，热点与协作状态进 Redis。",
            "source": "Redis 官方文档；《Redis 设计与实现》"
          },
          "l2": {
            "principles": [
              "单线程命令执行（简化并发）+ IO 多路",
              "一切皆 key + 结构",
              "过期与淘汰策略管理内存"
            ],
            "terms": [
              "RDB / AOF",
              "淘汰策略 LRU/LFU",
              "Pub/Sub / Stream",
              "Cluster 分片"
            ],
            "topics": [
              {
                "name": "存什么",
                "text": "缓存对象、计数器、会话、排行榜分数、消息 Stream——不是全量历史订单明细。"
              },
              {
                "name": "语法（命令）",
                "text": "GET/SET、HGET、ZADD、XADD…",
                "code": "SET user:88:profile '{\"vip\":1}' EX 3600\nZADD rank:game 99.5 user:88\nHSET session:abc user_id 88"
              },
              {
                "name": "快在哪",
                "text": "内存寻址 + 高效结构；适合热点与协作。"
              },
              {
                "name": "不适用",
                "text": "大范围分析扫描、强持久唯一真相、超大 value 滥用。"
              }
            ],
            "deps": {
              "upstream": [
                "数据库"
              ],
              "downstream": [
                "实时数据",
                "机器学习/算法"
              ]
            },
            "source": "https://redis.io/docs/"
          },
          "l3": {
            "tools": [
              "redis-cli",
              "RedisInsight",
              "Lettuce/Jedis"
            ],
            "code": "SETNX lock:order:1001 1 EX 10\nINCR rate:api:88",
            "resources": [
              {
                "name": "Redis Commands",
                "note": "https://redis.io/commands/"
              },
              {
                "name": "《Redis 设计与实现》",
                "note": "机械工业出版社"
              }
            ],
            "source": "Redis Documentation"
          }
        }
      },
      {
        "id": "clickhouse",
        "name": "ClickHouse",
        "tagline": "列式 OLAP · 海量聚合",
        "compare": {
          "type": "列式分析库",
          "workload": "OLAP 大批量扫描聚合",
          "storage": "列存压缩；MergeTree 家族",
          "language": "SQL（分析方言）",
          "data": "事件/日志/订单明细宽表",
          "pros": "扫描与聚合极快、压缩比高、物化视图强",
          "cons": "高频点更新/重事务弱；运维与表引擎需学习",
          "scenario": "实时数仓、行为分析、看板加速层"
        },
        "detail": {
          "subtitle": "面向分析的列式 DBMS",
          "l1": {
            "definition": "ClickHouse 是为 OLAP 设计的列式数据库，擅长对海量明细做聚合。",
            "why": "MySQL 扛不住的明细即席分析，常同步到 CH 做加速。",
            "source": "ClickHouse 官方文档"
          },
          "l2": {
            "principles": [
              "列存只读需要的列",
              "MergeTree 分区+排序键决定性能",
              "尽量追加写入，少做行级更新"
            ],
            "terms": [
              "MergeTree",
              "分区键 / ORDER BY",
              "物化视图",
              "投影 Projection"
            ],
            "topics": [
              {
                "name": "存什么",
                "text": "点击流、日志、交易明细等追加型事实。"
              },
              {
                "name": "语法",
                "text": "SQL；注意引擎与类型（DateTime64 等）。",
                "code": "SELECT toDate(event_time) dt, region, sum(amount) gmv\nFROM events\nWHERE event_time >= today()-7\nGROUP BY dt, region;"
              },
              {
                "name": "适用 / 不适用",
                "text": "适用：聚合分析。不适用：作为下单主库、频繁单行 UPDATE。"
              }
            ],
            "deps": {
              "upstream": [
                "Kafka",
                "ETL"
              ],
              "downstream": [
                "BI",
                "指标/语义层"
              ]
            },
            "source": "https://clickhouse.com/docs"
          },
          "l3": {
            "tools": [
              "clickhouse-client",
              "Kafka Engine",
              "物化视图"
            ],
            "code": "CREATE TABLE events (...) ENGINE=MergeTree PARTITION BY toYYYYMM(event_time) ORDER BY (region, event_time);",
            "resources": [
              {
                "name": "ClickHouse Docs",
                "note": "官方"
              }
            ],
            "source": "ClickHouse Documentation"
          }
        }
      },
      {
        "id": "milvus",
        "name": "Milvus",
        "tagline": "向量库 · ANN 相似度检索",
        "compare": {
          "type": "向量数据库",
          "workload": "近似最近邻检索 ANN",
          "storage": "向量索引（IVF/HNSW 等）+ 标量",
          "language": "SDK/API（集合、search）",
          "data": "Embedding 向量 + 元数据",
          "pros": "专为大规模向量检索优化",
          "cons": "不是通用事务库；需配合 Embedding 管线",
          "scenario": "推荐召回、语义搜索、RAG"
        },
        "detail": {
          "subtitle": "开源向量数据库",
          "l1": {
            "definition": "Milvus 存储高维向量并提供近似最近邻搜索，服务语义检索与推荐召回。",
            "why": "LLM/推荐把内容变成向量后，需要专用检索引擎。",
            "source": "Milvus 官方文档；《智能数据工程》"
          },
          "l2": {
            "principles": [
              "索引类型权衡召回与延迟",
              "标量过滤 + 向量检索",
              "与 Embedding 模型流水线解耦"
            ],
            "terms": [
              "Embedding",
              "IVF / HNSW",
              "Collection / Partition",
              "召回率 Recall"
            ],
            "topics": [
              {
                "name": "存什么",
                "text": "文本/图像/用户表征向量及商品 ID 等标量字段。"
              },
              {
                "name": "语法",
                "text": "Python/Java SDK：create_collection → insert → search。"
              },
              {
                "name": "适用 / 不适用",
                "text": "适用：相似度检索。不适用：替代业务 OLTP。"
              }
            ],
            "deps": {
              "upstream": [
                "Python",
                "机器学习/算法"
              ],
              "downstream": [
                "BI"
              ]
            },
            "source": "https://milvus.io/docs"
          },
          "l3": {
            "tools": [
              "pymilvus",
              "Attu",
              "Zilliz Cloud"
            ],
            "code": "# search(collection, data=[vec], limit=5, param={...})",
            "resources": [
              {
                "name": "Milvus Docs",
                "note": "官方"
              }
            ],
            "source": "Milvus Documentation"
          }
        }
      },
      {
        "id": "pgvector",
        "name": "pgvector",
        "tagline": "PG 内向量扩展 · SQL 检索",
        "compare": {
          "type": "关系库扩展（向量）",
          "workload": "中小规模向量 + 事务同库",
          "storage": "PostgreSQL 表内 vector 列",
          "language": "SQL（<=> 等距离运算符）",
          "data": "向量列 + 业务表同库关联",
          "pros": "与业务数据同事务/同 SQL；运维统一",
          "cons": "超大规模向量不如专用库",
          "scenario": "RAG 入门、同库关联检索、中小语料"
        },
        "detail": {
          "subtitle": "PostgreSQL 向量扩展",
          "l1": {
            "definition": "pgvector 为 PostgreSQL 增加 vector 类型与相似度运算符/索引。",
            "why": "不想引入独立向量集群时，先用 pgvector 验证业务。",
            "source": "pgvector GitHub / PostgreSQL 生态"
          },
          "l2": {
            "principles": [
              "向量即列，可 JOIN 业务表",
              "ivfflat/hnsw 索引加速",
              "仍受 PG 单集群规模约束"
            ],
            "terms": [
              "vector 类型",
              "<=> 余弦/L2",
              "ivfflat / hnsw"
            ],
            "topics": [
              {
                "name": "语法示例",
                "text": "ORDER BY embedding <=> query_vec LIMIT k",
                "code": "SELECT id, content FROM docs\nORDER BY embedding <=> '[0.12,0.03]'::vector\nLIMIT 5;"
              },
              {
                "name": "适用 / 不适用",
                "text": "适用：百万级内。不适用：十亿级向量——迁 Milvus 等。"
              }
            ],
            "deps": {
              "upstream": [
                "PostgreSQL",
                "Python"
              ],
              "downstream": [
                "机器学习/算法"
              ]
            },
            "source": "https://github.com/pgvector/pgvector"
          },
          "l3": {
            "tools": [
              "CREATE EXTENSION vector",
              "hnsw 索引"
            ],
            "code": "CREATE INDEX ON docs USING hnsw (embedding vector_cosine_ops);",
            "resources": [
              {
                "name": "pgvector README",
                "note": "GitHub"
              }
            ],
            "source": "pgvector project"
          }
        }
      },
      {
        "id": "elasticsearch",
        "name": "Elasticsearch",
        "tagline": "倒排检索 · 日志与搜索",
        "compare": {
          "type": "搜索引擎 / 文档检索",
          "workload": "全文检索与日志分析",
          "storage": "倒排索引（Lucene）",
          "language": "Query DSL（JSON）；ES|QL 演进中",
          "data": "文档、日志、商品文本",
          "pros": "全文检索强、聚合分析、生态（ELK）",
          "cons": "不是事务库；集群运维复杂；近实时非强一致",
          "scenario": "站内搜索、日志检索、可观测性"
        },
        "detail": {
          "subtitle": "基于 Lucene 的分布式搜索引擎",
          "l1": {
            "definition": "Elasticsearch 用倒排索引提供全文搜索与日志分析能力。",
            "why": "数据库 LIKE 扛不住的搜索与日志排查，交给 ES。",
            "source": "Elastic 官方文档"
          },
          "l2": {
            "principles": [
              "倒排索引加速词项查找",
              "近实时刷新（refresh）",
              "分片副本分布"
            ],
            "terms": [
              "Index / Shard",
              "倒排索引",
              "Query DSL",
              "ILM 生命周期"
            ],
            "topics": [
              {
                "name": "存什么",
                "text": "商品标题描述、日志行、文章——文本检索为主。"
              },
              {
                "name": "语法",
                "text": "JSON Query DSL；match/term/bool/aggs。"
              },
              {
                "name": "适用 / 不适用",
                "text": "适用：搜索与日志。不适用：订单账务真相库。"
              }
            ],
            "deps": {
              "upstream": [
                "ETL",
                "实时数据"
              ],
              "downstream": [
                "BI"
              ]
            },
            "source": "https://www.elastic.co/guide/"
          },
          "l3": {
            "tools": [
              "Kibana",
              "Logstash / Beats",
              "OpenSearch（兼容分支）"
            ],
            "code": "GET /products/_search\n{\"query\":{\"match\":{\"title\":\"无线耳机\"}}}",
            "resources": [
              {
                "name": "Elastic Docs",
                "note": "官方"
              }
            ],
            "source": "Elastic Documentation"
          }
        }
      }
    ]
  },
  {
    "id": "sql",
    "name": "SQL",
    "category": "process",
    "detail": {
      "subtitle": "结构化查询语言 · 数据读写与建模的通用接口",
      "l1": {
        "definition": "SQL 是管理关系型数据的标准化语言，覆盖查询、定义、操纵、控制与事务。",
        "why": "仓表、报表与特征生产几乎都落在 SQL；是数据从业者的「普通话」。",
        "source": "ISO/IEC 9075；《数据库系统概念》"
      },
      "l2": {
        "principles": [
          "声明式：描述结果，优化器选计划",
          "集合思维：JOIN/子查询/窗口",
          "分层：DDL→DML→DQL→优化"
        ],
        "terms": [
          "DDL/DML/DQL/DCL/TCL",
          "EXPLAIN",
          "覆盖索引",
          "窗口函数",
          "物化视图"
        ],
        "topics": [
          {
            "name": "增删改查 CRUD",
            "text": "DML 是日常读写的核心；注意事务边界与幂等。",
            "children": [
              {
                "name": "INSERT 写入",
                "text": "单行/批量插入；UPSERT（ON CONFLICT / ON DUPLICATE KEY）处理幂等。",
                "methods": [
                  "INSERT INTO t (...) VALUES (...)",
                  "INSERT ... SELECT ...",
                  "ON CONFLICT DO UPDATE（PG）"
                ],
                "code": "INSERT INTO orders(order_id,user_id,amount,created_at)\nVALUES (1001,88,199.00,NOW());"
              },
              {
                "name": "SELECT 查询",
                "text": "投影、过滤、排序、分页；优先谓词下推与索引列。",
                "methods": [
                  "WHERE / AND / OR",
                  "ORDER BY / LIMIT",
                  "DISTINCT / GROUP BY"
                ]
              },
              {
                "name": "UPDATE / DELETE",
                "text": "必须带 WHERE；生产常用软删除；TRUNCATE 更快但不可回滚部分引擎。",
                "methods": [
                  "UPDATE ... SET ... WHERE ...",
                  "DELETE FROM ... WHERE ...",
                  "软删除：status='deleted'"
                ]
              }
            ]
          },
          {
            "name": "DDL 建表与约束",
            "text": "结构决定后续查询与数据质量上限。",
            "children": [
              {
                "name": "列类型与主键",
                "text": "整数/小数/时间/字符串；主键稳定不可变。",
                "methods": [
                  "CREATE TABLE",
                  "PRIMARY KEY",
                  "SERIAL / BIGSERIAL / IDENTITY"
                ]
              },
              {
                "name": "约束",
                "text": "UNIQUE / FOREIGN KEY / CHECK / NOT NULL；约束是数据契约。",
                "code": "CREATE TABLE orders (\n  order_id BIGINT PRIMARY KEY,\n  amount DECIMAL(18,2) CHECK (amount>=0),\n  user_id BIGINT NOT NULL\n);"
              },
              {
                "name": "索引",
                "text": "B-Tree 默认；覆盖索引；避免低选择性列单独建索引。",
                "methods": [
                  "CREATE INDEX",
                  "复合索引最左前缀",
                  "EXPLAIN 验证是否走索引"
                ]
              }
            ]
          },
          {
            "name": "视图与对象",
            "text": "封装复杂度与权限边界。",
            "children": [
              {
                "name": "逻辑视图 VIEW",
                "text": "不存数据，每次查底层；适合权限与口径封装。"
              },
              {
                "name": "物化视图",
                "text": "预计算结果；需刷新策略（定时/触发）。"
              },
              {
                "name": "CTE 公共表表达式",
                "text": "WITH ... AS 提升可读性；部分引擎可物化 CTE。",
                "code": "WITH paid AS (\n  SELECT * FROM orders WHERE status='paid'\n)\nSELECT user_id, SUM(amount) FROM paid GROUP BY 1;"
              }
            ]
          },
          {
            "name": "窗口函数",
            "text": "在结果集上做排序/累计/前后行对比，不必自连接。",
            "children": [
              {
                "name": "排名类",
                "text": "ROW_NUMBER / RANK / DENSE_RANK",
                "methods": [
                  "PARTITION BY 分组",
                  "ORDER BY 排序键"
                ]
              },
              {
                "name": "分析类",
                "text": "SUM() OVER / LAG / LEAD / 移动平均",
                "code": "SELECT user_id, amount,\n  RANK() OVER (PARTITION BY user_id ORDER BY amount DESC) rnk,\n  LAG(amount) OVER (PARTITION BY user_id ORDER BY created_at) prev_amt\nFROM orders;"
              }
            ]
          },
          {
            "name": "优化与执行计划",
            "text": "先看计划再改 SQL。",
            "children": [
              {
                "name": "EXPLAIN / EXPLAIN ANALYZE",
                "text": "看扫描类型、连接方式、行数估计、实际耗时。",
                "methods": [
                  "Seq Scan vs Index Scan",
                  "Hash Join / Nested Loop / Merge Join",
                  "代价与行数偏差"
                ]
              },
              {
                "name": "常见优化手法",
                "text": "避免 SELECT *；避免对索引列函数；分区裁剪；谓词下推；统计信息更新。"
              },
              {
                "name": "统计与维护",
                "text": "ANALYZE；重建膨胀索引；大表分区。"
              }
            ]
          }
        ],
        "deps": {
          "upstream": [
            "数据库",
            "数据仓库"
          ],
          "downstream": [
            "BI",
            "ETL",
            "数据建模",
            "机器学习/算法",
            "指标/语义层"
          ]
        },
        "source": "Codd 关系模型；PostgreSQL / MySQL 文档"
      },
      "l3": {
        "tools": [
          "PostgreSQL",
          "MySQL",
          "ClickHouse",
          "Hive/Spark SQL",
          "BigQuery/Doris"
        ],
        "code": "CREATE TABLE orders (\n  order_id BIGINT PRIMARY KEY,\n  user_id BIGINT NOT NULL,\n  amount DECIMAL(18,2) CHECK (amount>=0),\n  status VARCHAR(16) DEFAULT 'created',\n  created_at TIMESTAMP NOT NULL\n);\nCREATE INDEX idx_user_dt ON orders(user_id, created_at);\nINSERT INTO orders VALUES (1001,88,199.00,'created',NOW());\nUPDATE orders SET status='paid' WHERE order_id=1001;\nCREATE VIEW v_paid AS SELECT * FROM orders WHERE status='paid';\nEXPLAIN\nSELECT user_id, amount,\n  RANK() OVER (PARTITION BY user_id ORDER BY amount DESC) rnk\nFROM orders\nWHERE created_at >= CURRENT_DATE - INTERVAL '30' DAY;",
        "resources": [
          {
            "name": "PostgreSQL Docs",
            "note": "https://www.postgresql.org/docs/"
          },
          {
            "name": "《SQL 必知必会》",
            "note": "人民邮电出版社"
          },
          {
            "name": "Use The Index, Luke",
            "note": "索引专题"
          }
        ],
        "source": "PostgreSQL Documentation；《SQL 必知必会》"
      }
    }
  },
  {
    "id": "python",
    "name": "Python",
    "category": "process",
    "detail": {
      "subtitle": "数据分析 / 工程编排的通用语言 · 可下钻知识树",
      "l1": {
        "definition": "Python 高可读；数据领域覆盖探索（Notebook+pandas）、生产流水线与轻量 GUI（Streamlit）。",
        "why": "一份技能覆盖取数→清洗→分析→出图→建模→部署。",
        "source": "PSF 文档；《利用 Python 进行数据分析》（机械工业出版社）"
      },
      "l2": {
        "principles": [
          "向量化优先，少写行循环",
          "探索与生产分离",
          "环境可复现",
          "出入有边界：幂等写仓、密钥走环境变量"
        ],
        "terms": [
          "DataFrame",
          "Series",
          "向量化",
          "Jupyter",
          "IDE",
          "Streamlit/Gradio"
        ],
        "topics": [
          {
            "name": "A. 用 Python 做数据分析的标准流程",
            "text": "不是「打开 IDE 就写模型」，而是固定闭环。",
            "children": [
              {
                "name": "A1 提出问题与指标",
                "text": "先写清业务问题、成功指标、时间范围与人群口径。"
              },
              {
                "name": "A2 取数 Ingest",
                "text": "SQLAlchemy/read_sql、仓、Parquet、API；大表下推过滤。"
              },
              {
                "name": "A3 探查 EDA",
                "text": "shape/dtypes/缺失率/分布；info、value_counts、isna().mean()。"
              },
              {
                "name": "A4 清洗与变换",
                "text": "类型、时间、去重、缺失、melt/pivot、merge；产出分析就绪表。"
              },
              {
                "name": "A5 聚合与可视化",
                "text": "groupby；matplotlib/seaborn/plotly；指标+对比+归因。"
              },
              {
                "name": "A6 固化与复用",
                "text": "函数/模块；写回表；需要时包 Streamlit。"
              }
            ]
          },
          {
            "name": "B. 数据分析常用库地图",
            "text": "掌握「库干什么 + 高频方法」比背全 API 重要。",
            "children": [
              {
                "name": "B1 pandas",
                "text": "内存表 DataFrame：读写清洗合并聚合。",
                "methods": [
                  "read_csv/parquet/sql — 读入",
                  "head/info/describe — 探查",
                  "isna/fillna/dropna — 缺失",
                  "astype/to_datetime — 类型",
                  "loc/iloc/query — 筛选",
                  "merge/concat — 关联",
                  "groupby().agg() — 聚合",
                  "pivot_table/melt — 透视",
                  "drop_duplicates — 去重",
                  "to_parquet/to_sql — 写出"
                ],
                "code": "import pandas as pd\ndf=pd.read_parquet('orders.parquet')\ndf['dt']=pd.to_datetime(df['created_at']).dt.date\ngmv=(df.query(\"status=='paid'\").groupby('dt',as_index=False)['amount'].sum())"
              },
              {
                "name": "B2 NumPy",
                "text": "ndarray 向量化；分位、掩码、广播。",
                "methods": [
                  "np.mean/std/percentile",
                  "np.where",
                  "broadcasting"
                ]
              },
              {
                "name": "B3 matplotlib",
                "text": "底层绑图：plot/bar/scatter/hist。",
                "methods": [
                  "plt.subplots OO 接口",
                  "savefig"
                ]
              },
              {
                "name": "B4 seaborn",
                "text": "统计图：分布、类别对比、热力。",
                "methods": [
                  "histplot/boxplot/barplot/lineplot/heatmap"
                ]
              },
              {
                "name": "B5 plotly",
                "text": "交互图；适合塞进 Streamlit。",
                "methods": [
                  "px.line/bar/scatter",
                  "write_html"
                ]
              },
              {
                "name": "B6 SciPy / statsmodels",
                "text": "检验与回归摘要。",
                "methods": [
                  "ttest_ind",
                  "chi2_contingency",
                  "OLS.summary()"
                ]
              },
              {
                "name": "B7 scikit-learn",
                "text": "划分、预处理、Pipeline、评估。",
                "methods": [
                  "train_test_split",
                  "StandardScaler",
                  "Pipeline",
                  "roc_auc_score"
                ]
              },
              {
                "name": "B8 SQLAlchemy",
                "text": "create_engine + read_sql / to_sql。"
              },
              {
                "name": "B9 requests",
                "text": "拉 API JSON；timeout 与重试。"
              },
              {
                "name": "B10 Polars / PySpark",
                "text": "更大表：Polars 单机加速；Spark 分布式。"
              }
            ]
          },
          {
            "name": "C. Python 图形界面三类（易混）",
            "text": "IDE ≠ Notebook ≠ 业务 GUI。",
            "children": [
              {
                "name": "C1 Jupyter 家族",
                "text": "分析师实验台：单元格+图+说明。"
              },
              {
                "name": "C2 IDE",
                "text": "PyCharm / VS Code / Spyder —— 写代码的 GUI。"
              },
              {
                "name": "C3 Web 小应用",
                "text": "Streamlit / Gradio / Dash 给别人点选。",
                "code": "import streamlit as st\ndf=...  # 数据\nregion=st.selectbox('地区', df['region'].unique())\nst.line_chart(df.query('region==@region').set_index('dt')['gmv'])"
              },
              {
                "name": "C4 桌面 GUI",
                "text": "Tkinter/PyQt；数据岗更常用 Streamlit。"
              }
            ]
          },
          {
            "name": "D. 高频代码模式",
            "text": "模板化提速。",
            "children": [
              {
                "name": "D1 日聚合",
                "text": "to_datetime → date → groupby → sum/count。"
              },
              {
                "name": "D2 漏斗",
                "text": "分步 merge 或 pivot；步转化率。"
              },
              {
                "name": "D3 留存",
                "text": "cohort × day_n 透视。",
                "methods": [
                  "首日 min(event_time)",
                  "间隔 dt.days",
                  "pivot_table nunique"
                ]
              },
              {
                "name": "D4 与 SQL 协作",
                "text": "大聚合下推仓；Python 做灵活分析与可视化。"
              }
            ]
          },
          {
            "name": "E. 工程与质量底线",
            "text": "脚本也要能复跑。",
            "children": [
              {
                "name": "E1 环境",
                "text": "venv/conda + requirements/lock。"
              },
              {
                "name": "E2 结构",
                "text": "config/io/transform/analyze/main；密钥不进 Git。"
              },
              {
                "name": "E3 校验",
                "text": "主键唯一、行数、空值率；GE / assert。"
              }
            ]
          }
        ],
        "deps": {
          "upstream": [
            "数据库",
            "ETL",
            "大数据平台",
            "实时数据"
          ],
          "downstream": [
            "BI",
            "机器学习/算法",
            "特征工程",
            "数据建模"
          ]
        },
        "source": "《利用 Python 进行数据分析》；pandas/NumPy/seaborn/Streamlit 文档；PEP 8"
      },
      "l3": {
        "tools": [
          "pandas/NumPy",
          "matplotlib/seaborn/plotly",
          "SciPy/statsmodels",
          "scikit-learn",
          "SQLAlchemy",
          "Jupyter/VS Code/PyCharm",
          "Streamlit/Dash",
          "Polars/PySpark",
          "Airflow/Prefect"
        ],
        "code": "import pandas as pd\nimport seaborn as sns\nfrom sqlalchemy import create_engine\nengine=create_engine('postgresql+psycopg2://readonly:@localhost/dw')\ndf=pd.read_sql(\"SELECT user_id,region,amount,created_at,status FROM dwd.orders WHERE created_at>=CURRENT_DATE-INTERVAL '30 day'\",con=engine)\ndf['created_at']=pd.to_datetime(df['created_at']); df['dt']=df['created_at'].dt.date\npaid=df.query(\"status=='paid' and amount>0\").drop_duplicates()\ndaily=paid.groupby(['dt','region'],as_index=False).agg(gmv=('amount','sum'),orders=('amount','size'))\nsns.lineplot(data=daily,x='dt',y='gmv',hue='region')",
        "resources": [
          {
            "name": "《利用 Python 进行数据分析》",
            "note": "机械工业出版社"
          },
          {
            "name": "pandas 用户指南",
            "note": "官方"
          },
          {
            "name": "Streamlit Docs",
            "note": "https://docs.streamlit.io/"
          }
        ],
        "source": "机械工业出版社；pandas / Streamlit 官方文档"
      }
    }
  },
  {
    "id": "etl",
    "name": "ETL",
    "category": "process",
    "detail": {
      "subtitle": "Extract-Transform-Load · 数据集成流水线",
      "l1": {
        "definition": "ETL/ELT 将多源数据抽取、转换后加载到仓/湖，是分析与智能的原料产线。",
        "why": "没有稳定集成，就没有可信报表与特征；它是「脏数据→可用表」的工厂。",
        "source": "《数据仓库工具箱》；Airbyte/dbt 文档"
      },
      "l2": {
        "principles": [
          "批与流互补：日批打底，流补新鲜度",
          "ELT 流行：先入湖/仓再 SQL/dbt 变换",
          "幂等与回补是生产生命线"
        ],
        "terms": [
          "抽取 CDC/全量",
          "缓慢变化维 SCD",
          "幂等写入",
          "数据契约",
          "回填 Backfill"
        ],
        "topics": [
          {
            "name": "抽取 Extract",
            "text": "JDBC 全量、增量时间戳、CDC（Debezium）日志解析。",
            "children": [
              {
                "name": "全量 vs 增量",
                "text": "全量简单；增量要水位与乱序。"
              },
              {
                "name": "CDC",
                "text": "基于 binlog/WAL，低侵入近实时。"
              }
            ]
          },
          {
            "name": "转换 Transform",
            "text": "清洗、标准化、关联、轻度聚合；质量规则嵌入。",
            "children": [
              {
                "name": "dbt 模式",
                "text": "SELECT 模型分层 staging→intermediate→mart。",
                "methods": [
                  "ref()/source()",
                  "测试 unique/not_null",
                  "文档生成"
                ]
              },
              {
                "name": "业务规则",
                "text": "币种、时区、主键、枚举映射。"
              }
            ]
          },
          {
            "name": "加载 Load",
            "text": "覆盖、追加、合并（MERGE）；分区覆盖保证幂等。"
          },
          {
            "name": "运维",
            "text": "失败重试、告警、血缘登记、SLA。"
          }
        ],
        "deps": {
          "upstream": [
            "数据库",
            "SQL",
            "Python",
            "实时数据",
            "调度编排"
          ],
          "downstream": [
            "大数据平台",
            "数据仓库",
            "数据湖",
            "数据血缘",
            "数据质量"
          ]
        },
        "source": "Kimball；dbt Docs；Debezium"
      },
      "l3": {
        "tools": [
          "Airflow/Dagster",
          "dbt",
          "Airbyte/Fivetran",
          "Spark",
          "Flink CDC",
          "DataX"
        ],
        "code": "-- dbt model example\n-- models/marts/fct_orders.sql\nSELECT o.order_id, o.user_id, o.amount, o.status, o.created_at\nFROM {{ ref('stg_orders') }} o\nWHERE o.amount >= 0",
        "resources": [
          {
            "name": "dbt Documentation",
            "note": "官方"
          },
          {
            "name": "Kimball Toolkit",
            "note": "维度建模与ETL"
          }
        ],
        "source": "dbt Labs；Kimball Group"
      }
    }
  },
  {
    "id": "realtime",
    "name": "实时数据",
    "category": "process",
    "catalog": true,
    "catalogLabel": "引擎列表",
    "detail": {
      "subtitle": "流式采集与计算 · Kafka / Flink 等",
      "l1": {
        "definition": "实时数据体系用事件流承载变更，用流引擎做低延迟计算与同步。",
        "why": "支撑风控、实时看板、在线特征；补齐批处理的新鲜度缺口。",
        "source": "《流式系统》；Kafka/Flink 官方文档"
      },
      "l2": {
        "principles": [
          "日志总线解耦生产消费",
          "事件时间与水位线处理乱序",
          "端到端语义：至少一次/精确一次"
        ],
        "terms": [
          "Topic",
          "Consumer Group",
          "Watermark",
          "Checkpoint",
          "CDC"
        ],
        "topics": [
          {
            "name": "架构分层",
            "text": "采集→总线（Kafka）→计算（Flink）→服务/仓表。"
          },
          {
            "name": "批流一体",
            "text": "同一套 SQL/语义覆盖历史回放与实时。"
          },
          {
            "name": "常见陷阱",
            "text": "乱序、重复投递、状态膨胀、热点分区。"
          }
        ],
        "deps": {
          "upstream": [
            "数据库"
          ],
          "downstream": [
            "ETL",
            "数据湖",
            "数据仓库",
            "特征工程",
            "机器学习/算法"
          ]
        },
        "source": "Kafka / Flink Documentation；O'Reilly《流式系统》"
      },
      "l3": {
        "tools": [
          "Kafka",
          "Flink",
          "Pulsar",
          "Flink CDC",
          "Spark Structured Streaming"
        ],
        "code": "-- 见 Kafka / Flink 引擎专页",
        "resources": [
          {
            "name": "Kafka Docs",
            "note": "官方"
          },
          {
            "name": "Flink Docs",
            "note": "官方"
          }
        ],
        "source": "Apache 官方文档"
      }
    },
    "engines": [
      {
        "id": "kafka",
        "name": "Apache Kafka",
        "tagline": "分布式日志 · 事件总线",
        "compare": {
          "type": "消息 / 事件流平台",
          "workload": "高吞吐发布订阅",
          "storage": "磁盘追加日志（Topic-Partition）",
          "language": "Producer/Consumer API；Kafka Streams DSL",
          "data": "事件、CDC、日志、指标流",
          "pros": "高吞吐、可回放、生态成熟（Connect）",
          "cons": "不是数据库；语义与顺序需设计；运维有门槛",
          "scenario": "服务解耦、CDC、实时管道缓冲"
        },
        "detail": {
          "subtitle": "分布式提交日志",
          "l1": {
            "definition": "Kafka 以追加日志形式存储事件流，支持多消费者独立进度回放。",
            "why": "实时链路的「总线」：上游写入、下游 Flink/ETL 消费。",
            "source": "Kafka 官方文档；《Kafka 权威指南》"
          },
          "l2": {
            "principles": [
              "Partition 并行与顺序（分区内）",
              "Consumer Group 负载均衡",
              "磁盘顺序写换吞吐"
            ],
            "terms": [
              "Topic / Partition / Offset",
              "ISR",
              "Exactly-once（幂等+事务）",
              "Kafka Connect"
            ],
            "topics": [
              {
                "name": "存什么",
                "text": "变更事件、埋点、CDC 行镜像——保留策略按时间/大小。"
              },
              {
                "name": "关键设计",
                "text": "分区键（避免热点）、压缩主题、Schema Registry。",
                "methods": [
                  "key 选择影响顺序与倾斜",
                  "retention.ms",
                  "acks / idempotence"
                ]
              },
              {
                "name": "适用 / 不适用",
                "text": "适用：流缓冲与解耦。不适用：随意点查替代 DB。"
              }
            ],
            "deps": {
              "upstream": [
                "数据库"
              ],
              "downstream": [
                "Flink",
                "ETL",
                "数据湖"
              ]
            },
            "source": "https://kafka.apache.org/documentation/"
          },
          "l3": {
            "tools": [
              "Kafka Connect",
              "Schema Registry",
              "kcat"
            ],
            "code": "# 生产/消费概念：topic orders.cdc\n# 消费者组独立提交 offset",
            "resources": [
              {
                "name": "Kafka Docs",
                "note": "官方"
              },
              {
                "name": "《Kafka 权威指南》",
                "note": "人民邮电出版社"
              }
            ],
            "source": "Apache Kafka Documentation"
          }
        }
      },
      {
        "id": "flink",
        "name": "Apache Flink",
        "tagline": "有状态流计算引擎",
        "compare": {
          "type": "流处理引擎",
          "workload": "低延迟有状态计算",
          "storage": "状态后端（RocksDB 等）+ Checkpoint",
          "language": "DataStream API / SQL / CEP",
          "data": "无界事件流上的聚合、连接、窗口",
          "pros": "精确一次、事件时间、状态与窗口强大",
          "cons": "运维与调优复杂；反压与状态膨胀需治理",
          "scenario": "实时指标、风控特征、CDC 入湖入仓"
        },
        "detail": {
          "subtitle": "有状态大规模流处理",
          "l1": {
            "definition": "Flink 在无界数据流上做有状态计算，支持事件时间与 Exactly-once 语义。",
            "why": "把「分钟级/秒级」指标与特征从批处理中解放出来。",
            "source": "Flink 官方文档；《流式系统》"
          },
          "l2": {
            "principles": [
              "事件时间 vs 处理时间",
              "Checkpoint 实现容错",
              "窗口与水位线 Watermark"
            ],
            "terms": [
              "Watermark",
              "Checkpoint / Savepoint",
              "Keyed State",
              "Flink SQL"
            ],
            "topics": [
              {
                "name": "核心能力",
                "text": "窗口聚合、双流 JOIN、CEP、CDC（Flink CDC）。",
                "code": "SELECT user_id, TUMBLE_END(ts, INTERVAL '1' MINUTE) AS w,\n       SUM(amount) AS gmv\nFROM orders GROUP BY user_id, TUMBLE(ts, INTERVAL '1' MINUTE);"
              },
              {
                "name": "状态与运维",
                "text": "RocksDB 状态、增量 Checkpoint、反压监控。"
              },
              {
                "name": "适用 / 不适用",
                "text": "适用：实时管道。简单定时批可用调度+SQL 更省。"
              }
            ],
            "deps": {
              "upstream": [
                "Kafka"
              ],
              "downstream": [
                "数据仓库",
                "特征工程",
                "数据湖"
              ]
            },
            "source": "https://flink.apache.org/docs/"
          },
          "l3": {
            "tools": [
              "Flink SQL",
              "Flink CDC",
              "PyFlink"
            ],
            "code": "-- Flink SQL 窗口聚合见上",
            "resources": [
              {
                "name": "Flink Docs",
                "note": "官方"
              },
              {
                "name": "《流式系统》",
                "note": "O'Reilly / 中译"
              }
            ],
            "source": "Apache Flink Documentation"
          }
        }
      },
      {
        "id": "pulsar",
        "name": "Apache Pulsar",
        "tagline": "存算分离流平台（可选）",
        "compare": {
          "type": "消息流平台",
          "workload": "发布订阅 / 队列统一",
          "storage": "BookKeeper 存算分离",
          "language": "Client API；Functions",
          "data": "事件流、多租户主题",
          "pros": "多租户、存算分离、地理复制",
          "cons": "生态与人才相对 Kafka 少",
          "scenario": "云原生多租户消息、需强隔离的流平台"
        },
        "detail": {
          "subtitle": "云原生分布式消息流",
          "l1": {
            "definition": "Pulsar 将计算（Broker）与存储（BookKeeper）分离，强调多租户。",
            "why": "在多团队共享流平台时可评估。",
            "source": "Apache Pulsar 文档"
          },
          "l2": {
            "principles": [
              "Topic 多租户命名空间",
              "Cursor 消费位点",
              "与 Kafka API 兼容层"
            ],
            "terms": [
              "Tenant / Namespace",
              "BookKeeper",
              "Function"
            ],
            "topics": [
              {
                "name": "对比 Kafka",
                "text": "运维模型不同；选型看隔离与云原生需求。"
              }
            ],
            "deps": {
              "upstream": [
                "数据库"
              ],
              "downstream": [
                "Flink"
              ]
            },
            "source": "https://pulsar.apache.org/docs/"
          },
          "l3": {
            "tools": [
              "pulsar-admin",
              "Pulsar Functions"
            ],
            "code": "# pulsar-client produce ...",
            "resources": [
              {
                "name": "Pulsar Docs",
                "note": "官方"
              }
            ],
            "source": "Apache Pulsar"
          }
        }
      }
    ]
  },
  {
    "id": "orchestrate",
    "name": "调度编排",
    "category": "process",
    "detail": {
      "subtitle": "工作流调度 · DAG 依赖与 SLA",
      "l1": {
        "definition": "调度编排按依赖图定时/事件触发任务（ETL、质检、模型、报表），管理重试与告警。",
        "why": "是数据生产的「操作系统」：没有编排就只有手工脚本。",
        "source": "Airflow 官方文档；《数据密集型应用系统设计》中的批处理讨论"
      },
      "l2": {
        "principles": [
          "DAG 表达依赖，避免隐藏顺序",
          "幂等任务 + 清晰执行日期",
          "失败可观测：重试、告警、SLA"
        ],
        "terms": [
          "DAG",
          "Operator/Task",
          "Backfill",
          "Sensor",
          "SLA / SLA miss"
        ],
        "topics": [
          {
            "name": "编排模式",
            "text": "时间驱动 cron；传感器等待上游；事件驱动（数据集就绪）。",
            "children": [
              {
                "name": "Airflow",
                "text": "Python DAG；丰富 Operator；Worker 水平扩展。",
                "methods": [
                  "BashOperator/PythonOperator",
                  "TaskFlow API",
                  "XCom 慎用",
                  "Dataset 调度"
                ]
              },
              {
                "name": "Dagster / Prefect",
                "text": "软件定义资产 / 动态流；偏开发体验。"
              },
              {
                "name": "云原生",
                "text": "Argo Workflows、云厂商 Data Pipeline。"
              }
            ]
          },
          {
            "name": "设计原则",
            "text": "任务粒度适中；配置与代码分离；禁止巨型上帝 DAG。",
            "children": [
              {
                "name": "分区与执行日期",
                "text": "ds/logical_date 贯穿回补。"
              },
              {
                "name": "并发与队列",
                "text": "池化限制打爆源库。"
              }
            ]
          },
          {
            "name": "与质量/血缘",
            "text": "质检任务挂在关键节点后；运行元数据写入血缘。"
          }
        ],
        "deps": {
          "upstream": [
            "ETL",
            "Python"
          ],
          "downstream": [
            "数据仓库",
            "数据质量",
            "机器学习/算法",
            "BI"
          ]
        },
        "source": "Apache Airflow Docs；Dagster Docs"
      },
      "l3": {
        "tools": [
          "Apache Airflow",
          "Dagster",
          "Prefect",
          "DolphinScheduler",
          "Argo"
        ],
        "code": "from airflow import DAG\nfrom airflow.operators.bash import BashOperator\nwith DAG('daily_etl', schedule='@daily', catchup=False) as dag:\n    extract = BashOperator(task_id='extract', bash_command='python extract.py')\n    transform = BashOperator(task_id='transform', bash_command='dbt run')\n    extract >> transform",
        "resources": [
          {
            "name": "Airflow Documentation",
            "note": "官方"
          },
          {
            "name": "Dagster Docs",
            "note": "资产编排"
          }
        ],
        "source": "Apache Airflow；相关编排工具文档"
      }
    }
  },
  {
    "id": "bigdata",
    "name": "大数据平台",
    "category": "storage",
    "detail": {
      "subtitle": "分布式计算与存储平台 · Hadoop/Spark 生态",
      "l1": {
        "definition": "大数据平台用分布式存储与计算处理单机难以下咽的数据量与复杂度。",
        "why": "仓/湖的计算底座；批处理、交互 SQL、ML 训练常跑在其上。",
        "source": "《Hadoop 权威指南》；Spark 官方文档"
      },
      "l2": {
        "principles": [
          "数据本地性与并行分区",
          "存算分离渐成主流",
          "统一 SQL 层降低使用门槛"
        ],
        "terms": [
          "HDFS/对象存储",
          "YARN/K8s",
          "Spark Job",
          "分区裁剪",
          "Catalyst/Tungsten"
        ],
        "topics": [
          {
            "name": "存储层",
            "text": "HDFS 或 S3/OSS 对象存储；表格式 Iceberg/Hudi/Delta。",
            "children": [
              {
                "name": "文件格式",
                "text": "Parquet/ORC 列式；压缩与谓词下推。"
              }
            ]
          },
          {
            "name": "计算层 Spark",
            "text": "RDD/DataFrame/Spark SQL；批流 API。",
            "children": [
              {
                "name": "优化",
                "text": "广播连接、AQE、分区数、缓存慎用。",
                "methods": [
                  "repartition/coalesce",
                  "broadcast join",
                  "EXPLAIN"
                ]
              }
            ]
          },
          {
            "name": "交互 SQL",
            "text": "Hive/Presto/Trino/Spark Thrift；与 BI 对接。"
          },
          {
            "name": "平台治理",
            "text": "队列配额、小文件治理、成本账单。"
          }
        ],
        "deps": {
          "upstream": [
            "ETL",
            "调度编排",
            "实时数据"
          ],
          "downstream": [
            "数据仓库",
            "数据湖",
            "机器学习/算法",
            "数据血缘"
          ]
        },
        "source": "Spark Docs；Hadoop 生态文献"
      },
      "l3": {
        "tools": [
          "Spark",
          "Hive",
          "Trino/Presto",
          "HDFS/S3",
          "YARN/K8s",
          "Iceberg"
        ],
        "code": "from pyspark.sql import SparkSession\nspark=SparkSession.builder.appName('gmv').getOrCreate()\ndf=spark.read.parquet('s3a://lake/orders')\ndf.filter(\"status='paid'\").groupBy('region').sum('amount').show()",
        "resources": [
          {
            "name": "Apache Spark Docs",
            "note": "官方"
          },
          {
            "name": "《Hadoop 权威指南》",
            "note": "清华大学出版社相关引进版"
          }
        ],
        "source": "Apache Spark Documentation"
      }
    }
  },
  {
    "id": "dwh",
    "name": "数据仓库",
    "category": "storage",
    "detail": {
      "subtitle": "面向主题的分析型存储 · 单一事实来源",
      "l1": {
        "definition": "数据仓库按主题整合历史数据，支持管理决策与报表，强调一致性与可追溯。",
        "why": "BI/指标/特征的可信底座；与业务库解耦分析负载。",
        "source": "Inmon；Kimball《数据仓库工具箱》"
      },
      "l2": {
        "principles": [
          "主题导向与历史可追溯",
          "分层：ODS/DWD/DWS/ADS",
          "维度建模或范式仓库选型"
        ],
        "terms": [
          "事实表/维度表",
          "缓慢变化维 SCD",
          "总线矩阵",
          "数据金层"
        ],
        "topics": [
          {
            "name": "分层架构",
            "text": "贴源→明细清洗→汇总→应用。",
            "children": [
              {
                "name": "ODS",
                "text": "贴源，少变换，保真。"
              },
              {
                "name": "DWD",
                "text": "一致性清洗、统一枚举与时区。"
              },
              {
                "name": "DWS/ADS",
                "text": "轻度汇总与强应用指标。"
              }
            ]
          },
          {
            "name": "建模方法",
            "text": "Kimball 星型/雪花；Data Vault 等。",
            "children": [
              {
                "name": "事实类型",
                "text": "事务事实、周期快照、累积快照。"
              },
              {
                "name": "维度设计",
                "text": "代理键、退化维、角色扮演维。"
              }
            ]
          },
          {
            "name": "与湖关系",
            "text": "仓管高价值结构化；湖存原始与多样；湖仓一体融合。"
          }
        ],
        "deps": {
          "upstream": [
            "ETL",
            "大数据平台",
            "数据湖",
            "数据建模"
          ],
          "downstream": [
            "BI",
            "指标/语义层",
            "机器学习/算法",
            "数据治理"
          ]
        },
        "source": "Kimball Group；Inmon"
      },
      "l3": {
        "tools": [
          "Snowflake/BigQuery/Redshift",
          "Doris/StarRocks",
          "Hive+Iceberg",
          "dbt"
        ],
        "code": "-- 星型：fct_sales + dim_date + dim_customer\nSELECT d.month, c.region, SUM(f.amount) gmv\nFROM fct_sales f\nJOIN dim_date d ON f.date_key=d.date_key\nJOIN dim_customer c ON f.customer_key=c.customer_key\nGROUP BY 1,2;",
        "resources": [
          {
            "name": "《数据仓库工具箱》",
            "note": "电子工业出版社"
          },
          {
            "name": "dbt Best Practices",
            "note": "官方指南"
          }
        ],
        "source": "Kimball Toolkit；云仓官方文档"
      }
    }
  },
  {
    "id": "lake",
    "name": "数据湖",
    "category": "storage",
    "detail": {
      "subtitle": "原始与多样数据的低成本存储 · 开放表格式",
      "l1": {
        "definition": "数据湖在廉价对象存储上保存原始/半结构化/结构化数据，用开放表格式提供事务与时间旅行。",
        "why": "解决「先落再建」与多引擎共用同一份数据；是湖仓一体的底座。",
        "source": "Databricks Delta；Apache Iceberg/Hudi 文档；《智能数据工程》"
      },
      "l2": {
        "principles": [
          "Schema-on-read 与演进治理并存",
          "表格式解决一致性与小文件",
          "多引擎（Spark/Trino/Flink）共享"
        ],
        "terms": [
          "对象存储",
          "Iceberg/Delta/Hudi",
          "时间旅行 Time Travel",
          "ACID 表",
          "数据湖仓 Lakehouse"
        ],
        "topics": [
          {
            "name": "为什么需要湖",
            "text": "源多且原始价值高；仓无法廉价承载全量原始。",
            "children": [
              {
                "name": "青铜/银/金分层",
                "text": "Medallion：Bronze 原始→Silver 清洗→Gold 产品。"
              }
            ]
          },
          {
            "name": "开放表格式",
            "text": "在 Parquet 上加元数据层：快照、分区演化、UPSERT。",
            "children": [
              {
                "name": "Iceberg",
                "text": "隐式分区、隐藏分区演进、丰富生态。"
              },
              {
                "name": "Delta Lake",
                "text": "与 Databricks/Spark 紧密；日志事务。"
              },
              {
                "name": "Hudi",
                "text": "近实时 upsert、增量消费强。"
              }
            ]
          },
          {
            "name": "治理挑战",
            "text": "变成数据沼泽：无目录、无质量、无权限。需目录+DQ+治理。",
            "methods": [
              "数据目录 Catalog",
              "生命周期冷热分层",
              "列级权限"
            ]
          },
          {
            "name": "湖仓一体",
            "text": "同一表既服务 BI SQL 又服务科学计算。"
          }
        ],
        "deps": {
          "upstream": [
            "ETL",
            "实时数据",
            "大数据平台"
          ],
          "downstream": [
            "数据仓库",
            "特征工程",
            "机器学习/算法",
            "数据治理"
          ]
        },
        "source": "Apache Iceberg Docs；Delta Lake Docs；清华《智能数据工程》"
      },
      "l3": {
        "tools": [
          "Apache Iceberg",
          "Delta Lake",
          "Apache Hudi",
          "AWS S3/OSS",
          "Spark/Flink/Trino"
        ],
        "code": "-- Iceberg SQL 示意\nCREATE TABLE lake.orders (...) USING iceberg PARTITIONED BY (days(ts));\nCALL system.snapshot(...); -- 时间旅行查询历史快照",
        "resources": [
          {
            "name": "Iceberg Documentation",
            "note": "官方"
          },
          {
            "name": "Delta Lake Docs",
            "note": "官方"
          },
          {
            "name": "《智能数据工程》",
            "note": "清华大学出版社"
          }
        ],
        "source": "Iceberg/Delta/Hudi 官方文档"
      }
    }
  },
  {
    "id": "modeling",
    "name": "数据建模",
    "category": "analyze",
    "detail": {
      "subtitle": "从业务到表结构的抽象 · 维度/范式/指标模型",
      "l1": {
        "definition": "数据建模把业务实体与过程抽象为可存储、可查询、可治理的数据结构。",
        "why": "模型质量决定取数效率、口径一致性与返工成本。",
        "source": "《数据仓库工具箱》；《数据库系统概念》ER 章"
      },
      "l2": {
        "principles": [
          "先业务语言后技术落表",
          "粒度是事实表第一决策",
          "一致性维度实现跨主题对齐"
        ],
        "terms": [
          "ER 模型",
          "维度建模",
          "粒度 Grain",
          "总线矩阵",
          "规范化 3NF"
        ],
        "topics": [
          {
            "name": "业务理解",
            "text": "写清实体、事件、指标口径与生命周期。"
          },
          {
            "name": "维度建模实务",
            "text": "选粒度→识别维度→事实→补齐缓慢变化。",
            "children": [
              {
                "name": "总线矩阵",
                "text": "过程×维度复用，避免烟囱。"
              },
              {
                "name": "SCD1/2/3",
                "text": "覆盖、历史行、有限历史列。"
              }
            ]
          },
          {
            "name": "仓外模型",
            "text": "操作库 3NF；文档模型；宽表反范仅为加速。"
          },
          {
            "name": "与指标层",
            "text": "原子指标落模型，派生交给语义层。"
          }
        ],
        "deps": {
          "upstream": [
            "数据库",
            "SQL",
            "数据仓库"
          ],
          "downstream": [
            "ETL",
            "BI",
            "指标/语义层",
            "机器学习/算法"
          ]
        },
        "source": "Kimball；Chen ER 模型"
      },
      "l3": {
        "tools": [
          "PowerDesigner/dbdiagram",
          "dbt",
          "LookML",
          "ER/Studio"
        ],
        "code": "-- grain: 一张订单行一日\n-- fct_order_item (order_id, item_id, date_key, qty, amount, ...)",
        "resources": [
          {
            "name": "《数据仓库工具箱》",
            "note": "Kimball"
          },
          {
            "name": "《数据库系统概念》",
            "note": "ER/关系"
          }
        ],
        "source": "Kimball Group；教材"
      }
    }
  },
  {
    "id": "bi",
    "name": "BI",
    "category": "analyze",
    "catalog": true,
    "catalogLabel": "工具列表",
    "detail": {
      "subtitle": "商业智能 · 指标可视化与自助分析",
      "l1": {
        "definition": "BI 通过语义层、报表与仪表盘，将可信数据转化为可行动洞察。",
        "why": "数据价值的消费面：统一口径下做决策。",
        "source": "Gartner BI；电子工业出版社 BI 相关教材"
      },
      "l2": {
        "principles": [
          "先指标后图表",
          "视觉编码：位置长度优先",
          "权限与性能：RLS、预聚合、缓存"
        ],
        "terms": [
          "Dashboard",
          "语义层",
          "OLAP",
          "下钻/切片",
          "RLS"
        ],
        "topics": [
          {
            "name": "指标体系",
            "text": "原子/派生/维度；避免同名不同义——详见「指标/语义层」。"
          },
          {
            "name": "仪表盘设计",
            "text": "一屏一主题；北向指标+归因；少而精。"
          },
          {
            "name": "自助 vs 治理",
            "text": "受控探索与固定报表平衡。"
          },
          {
            "name": "嵌入与订阅",
            "text": "邮件、告警、嵌入业务系统。"
          }
        ],
        "deps": {
          "upstream": [
            "数据仓库",
            "数据建模",
            "SQL",
            "数据治理",
            "指标/语义层"
          ],
          "downstream": [
            "机器学习/算法"
          ]
        },
        "source": "Kimball；Tableau/Power BI/Superset 文档"
      },
      "l3": {
        "tools": [
          "Tableau",
          "Power BI",
          "Superset",
          "Metabase",
          "Looker"
        ],
        "code": "SELECT region, DATE(created_at) dt, SUM(amount) paid_gmv\nFROM dws.order_1d WHERE status='paid' GROUP BY 1,2;",
        "resources": [
          {
            "name": "Apache Superset",
            "note": "开源 BI"
          },
          {
            "name": "Kimball Toolkit",
            "note": "报表与维"
          }
        ],
        "source": "各 BI 官方文档；Kimball"
      }
    },
    "engines": [
      {
        "id": "tableau",
        "name": "Tableau",
        "tagline": "交互可视分析标杆",
        "compare": {
          "type": "商业 BI",
          "workload": "可视化探索 / 仪表盘",
          "storage": "活查询或抽取 Extract",
          "language": "VizQL + 计算字段",
          "data": "仓表、文件、语义模型",
          "pros": "交互与视觉编码强、企业治理成熟",
          "cons": "授权成本；复杂指标需建模配合",
          "scenario": "分析师探索、高管看板"
        },
        "detail": {
          "subtitle": "交互式视觉分析平台",
          "l1": {
            "definition": "Tableau 通过拖拽维度度量生成可视化，底层是 VizQL。",
            "why": "适合专业分析师做深度探索与叙事看板。",
            "source": "Tableau 帮助文档；Few《Now You See It》"
          },
          "l2": {
            "principles": [
              "先数据模型后图表",
              "聚合在数据源侧尽量下推",
              "仪表盘应一屏一主题"
            ],
            "terms": [
              "Extract",
              "计算字段",
              "LOD 表达式",
              "行级安全"
            ],
            "topics": [
              {
                "name": "连接与建模",
                "text": "活连接 vs 抽取；关系模型/联合。"
              },
              {
                "name": "计算与 LOD",
                "text": "FIXED/INCLUDE/EXCLUDE 控制聚合粒度。"
              },
              {
                "name": "发布与权限",
                "text": "Server/Cloud；项目权限与认证。"
              }
            ],
            "deps": {
              "upstream": [
                "数据仓库",
                "指标/语义层"
              ],
              "downstream": []
            },
            "source": "Tableau Help"
          },
          "l3": {
            "tools": [
              "Tableau Desktop/Server",
              "Prep"
            ],
            "code": "-- 语义：paid_gmv = SUM(amount) WHERE status=paid",
            "resources": [
              {
                "name": "Tableau Docs",
                "note": "官方"
              }
            ],
            "source": "Tableau Documentation"
          }
        }
      },
      {
        "id": "powerbi",
        "name": "Power BI",
        "tagline": "微软生态自助 BI",
        "compare": {
          "type": "商业 BI",
          "workload": "自助报表 / 企业门户",
          "storage": "Import / DirectQuery / 复合",
          "language": "DAX + Power Query(M)",
          "data": "Excel到仓湖的广泛连接",
          "pros": "Office365 集成、DAX 指标灵活、性价比",
          "cons": "大数据 DirectQuery 需建模功力；容量规划",
          "scenario": "企业报表中心、部门自助"
        },
        "detail": {
          "subtitle": "微软商业智能套件",
          "l1": {
            "definition": "Power BI 用 Power Query 取数变换，用 DAX 定义指标与模型。",
            "why": "微软栈企业默认选择；与 Azure/Fabric 集成深。",
            "source": "Microsoft Learn · Power BI"
          },
          "l2": {
            "principles": [
              "星型模型是性能关键",
              "Import 与 DirectQuery 取舍",
              "度量值集中管理"
            ],
            "terms": [
              "DAX",
              "Power Query",
              "数据集 Dataset",
              "工作区"
            ],
            "topics": [
              {
                "name": "模型设计",
                "text": "维度表+事实表；避免雪花过度。"
              },
              {
                "name": "DAX 度量",
                "text": "CALCULATE、时间智能、迭代函数。",
                "code": "Paid GMV := CALCULATE(SUM(Orders[Amount]), Orders[Status]=\"paid\")"
              },
              {
                "name": "分发",
                "text": "工作区、应用、行级安全 RLS。"
              }
            ],
            "deps": {
              "upstream": [
                "数据仓库",
                "数据建模"
              ],
              "downstream": []
            },
            "source": "Microsoft Learn"
          },
          "l3": {
            "tools": [
              "Power BI Desktop",
              "Fabric",
              "Gateway"
            ],
            "code": "Paid GMV := CALCULATE(SUM(Orders[Amount]), Orders[Status]=\"paid\")",
            "resources": [
              {
                "name": "Microsoft Learn",
                "note": "Power BI"
              }
            ],
            "source": "Microsoft Documentation"
          }
        }
      },
      {
        "id": "superset",
        "name": "Apache Superset",
        "tagline": "开源 BI · SQL Lab",
        "compare": {
          "type": "开源 BI",
          "workload": "SQL 探索 + 仪表盘",
          "storage": "依赖后端仓/引擎",
          "language": "SQL + 虚拟数据集",
          "data": "任意 SQL 引擎",
          "pros": "免费、可二次开发、SQL Lab 友好",
          "cons": "企业治理/视觉精致度弱于商业产品",
          "scenario": "数据团队内部分析门户"
        },
        "detail": {
          "subtitle": "Apache 开源可视化平台",
          "l1": {
            "definition": "Superset 提供 SQL Lab、图表与仪表盘，对接多种 SQL 引擎。",
            "why": "开源栈与大数据引擎搭配常见。",
            "source": "Apache Superset Documentation"
          },
          "l2": {
            "principles": [
              "数据集 Dataset 抽象",
              "缓存与异步查询",
              "角色权限"
            ],
            "terms": [
              "SQL Lab",
              "Virtual Dataset",
              "Cache warmup",
              "RBAC"
            ],
            "topics": [
              {
                "name": "对接引擎",
                "text": "Presto/Trino、Hive、PG、ClickHouse 等。"
              },
              {
                "name": "仪表盘",
                "text": "过滤器作用域、下钻、CSS 模板。"
              }
            ],
            "deps": {
              "upstream": [
                "数据仓库",
                "大数据平台"
              ],
              "downstream": []
            },
            "source": "https://superset.apache.org/docs/"
          },
          "l3": {
            "tools": [
              "Docker Compose 部署",
              "Celery worker"
            ],
            "code": "SELECT region, SUM(amount) FROM dws.order_1d GROUP BY 1;",
            "resources": [
              {
                "name": "Superset Docs",
                "note": "官方"
              }
            ],
            "source": "Apache Superset"
          }
        }
      },
      {
        "id": "metabase",
        "name": "Metabase",
        "tagline": "轻量问答式 BI",
        "compare": {
          "type": "开源/商业 BI",
          "workload": "业务自助问答",
          "storage": "依赖源库",
          "language": "图形查询 + SQL",
          "data": "仓表/业务库只读",
          "pros": "上手极快、问题式探索友好",
          "cons": "超复杂语义层弱于 Looker",
          "scenario": "创业团队、业务自助轻量报表"
        },
        "detail": {
          "subtitle": "面向业务的简洁 BI",
          "l1": {
            "definition": "Metabase 强调「提问」式探索与简易仪表盘。",
            "why": "让业务少写 SQL 也能取数。",
            "source": "Metabase 文档"
          },
          "l2": {
            "principles": [
              "基于表的图形查询构建器",
              "可嵌入",
              "简单权限"
            ],
            "terms": [
              "Question",
              "Dashboard",
              "Pulse 订阅",
              "Sandboxing"
            ],
            "topics": [
              {
                "name": "模型与指标",
                "text": "用 Model 固化常用表；指标可用 SQL 保存。"
              },
              {
                "name": "嵌入",
                "text": "把图表嵌进产品后台。"
              }
            ],
            "deps": {
              "upstream": [
                "数据库",
                "数据仓库"
              ],
              "downstream": []
            },
            "source": "https://www.metabase.com/docs/"
          },
          "l3": {
            "tools": [
              "Metabase Cloud/OSS"
            ],
            "code": "-- saved question SQL",
            "resources": [
              {
                "name": "Metabase Docs",
                "note": "官方"
              }
            ],
            "source": "Metabase Documentation"
          }
        }
      },
      {
        "id": "looker",
        "name": "Looker",
        "tagline": "LookML 语义层 BI",
        "compare": {
          "type": "语义层驱动 BI",
          "workload": "受控自助 + 嵌入",
          "storage": "直连仓（无抽取为主）",
          "language": "LookML 建模语言",
          "data": "仓内受控视图",
          "pros": "指标口径代码化、Git 协作、治理强",
          "cons": "学习曲线；绑定建模投入",
          "scenario": "中大型企业统一口径与嵌入分析"
        },
        "detail": {
          "subtitle": "面向开发者的语义层 BI",
          "l1": {
            "definition": "Looker 用 LookML 定义维度度量与探索模型，再生成 SQL。",
            "why": "把指标治理做成代码评审流程。",
            "source": "Looker / Google Cloud 文档；Kimball"
          },
          "l2": {
            "principles": [
              "Explore 基于 view/model",
              "Git 版本管理 LookML",
              "PDT 持久派生表"
            ],
            "terms": [
              "LookML",
              "Explore",
              "PDT",
              "内容访问控制"
            ],
            "topics": [
              {
                "name": "LookML 核心",
                "text": "view、dimension、measure、explore。"
              },
              {
                "name": "与指标平台关系",
                "text": "语义层思想与独立指标平台相通。"
              }
            ],
            "deps": {
              "upstream": [
                "数据仓库",
                "指标/语义层",
                "数据建模"
              ],
              "downstream": []
            },
            "source": "Google Cloud Looker Docs"
          },
          "l3": {
            "tools": [
              "LookML IDE",
              "Content Validator"
            ],
            "code": "measure: paid_gmv { type: sum sql: ${amount};; filters: [status: paid] }",
            "resources": [
              {
                "name": "Looker Docs",
                "note": "Google Cloud"
              }
            ],
            "source": "Looker Documentation"
          }
        }
      }
    ]
  },
  {
    "id": "metrics",
    "name": "指标/语义层",
    "category": "analyze",
    "detail": {
      "subtitle": "统一口径的指标定义与消费抽象",
      "l1": {
        "definition": "指标/语义层把业务指标定义为可复用、可版本化的对象，屏蔽底层表差异，供 BI/API/实验共用。",
        "why": "解决「同名不同义」；让取数从抄 SQL 变为消费受控指标。",
        "source": "dbt Semantic Layer；LookML；头条/美团等指标平台实践分享"
      },
      "l2": {
        "principles": [
          "原子指标 + 修饰词 + 时间周期 = 派生指标",
          "维度可下钻对齐",
          "定义即代码，可评审可测试"
        ],
        "terms": [
          "原子指标",
          "派生指标",
          "维度",
          "语义层 Semantic Layer",
          "口径版本"
        ],
        "topics": [
          {
            "name": "指标体系设计",
            "text": "先盘点业务北极星与过程指标，再建技术对象。",
            "children": [
              {
                "name": "命名规范",
                "text": "谁的什么在何时何种条件下的度量。"
              },
              {
                "name": "一致性",
                "text": "同一 amount 在订单/支付主题的边界。"
              }
            ]
          },
          {
            "name": "语义层实现",
            "text": "LookML、Cube、dbt Metric、自研指标服务。",
            "children": [
              {
                "name": "生成 SQL",
                "text": "根据查询维度自动编译聚合查询。"
              },
              {
                "name": "缓存与物化",
                "text": "热点指标预计算。"
              }
            ]
          },
          {
            "name": "消费方式",
            "text": "BI 绑定、指标 API、特征派生、实验评估。"
          },
          {
            "name": "治理",
            "text": "负责人、变更评审、废弃策略、血缘到报表。"
          }
        ],
        "deps": {
          "upstream": [
            "数据仓库",
            "数据建模",
            "数据治理"
          ],
          "downstream": [
            "BI",
            "机器学习/算法",
            "特征工程"
          ]
        },
        "source": "dbt Semantic Layer Docs；Looker LookML；公开指标平台实践"
      },
      "l3": {
        "tools": [
          "dbt Metrics/Semantic Layer",
          "Cube.dev",
          "LookML",
          "自研指标平台"
        ],
        "code": "-- metric: paid_gmv\n-- type: sum\n-- sql: amount\n-- filters: status = 'paid'\n-- dimensions: dt, region, channel",
        "resources": [
          {
            "name": "dbt Semantic Layer",
            "note": "官方文档"
          },
          {
            "name": "Cube.dev Docs",
            "note": "语义 API"
          }
        ],
        "source": "dbt Labs；Cube；Looker"
      }
    }
  },
  {
    "id": "ml",
    "name": "机器学习/算法",
    "category": "intel",
    "detail": {
      "subtitle": "从数据学习模式并驱动预测与决策",
      "l1": {
        "definition": "机器学习用算法从数据发现模式，服务预测、推荐、分类、聚类与异常检测。",
        "why": "把沉淀数据变成自动化决策；依赖仓表、实时与特征质量。",
        "source": "《统计学习方法》（清华大学出版社）"
      },
      "l2": {
        "principles": [
          "监督/无监督/强化学习",
          "特征质量决定上限",
          "MLOps 闭环：训练-评估-部署-监控"
        ],
        "terms": [
          "损失函数",
          "过拟合/正则",
          "AUC/F1",
          "模型服务",
          "向量检索/RAG"
        ],
        "topics": [
          {
            "name": "经典算法",
            "text": "线性/逻辑回归、树模型 GBDT、聚类、协同过滤。",
            "children": [
              {
                "name": "树模型",
                "text": "XGBoost/LightGBM 表格数据主力。"
              },
              {
                "name": "评估",
                "text": "离线指标 + 校准；分类注意不平衡。"
              }
            ]
          },
          {
            "name": "样本与泄露",
            "text": "时间切分；禁止用未来信息。",
            "children": [
              {
                "name": "偏差",
                "text": "选择偏差、标签延迟。"
              }
            ]
          },
          {
            "name": "深度学习 / LLM",
            "text": "表征学习；RAG 需向量库。"
          },
          {
            "name": "上线与监控",
            "text": "特征漂移、效果衰减、回滚。"
          }
        ],
        "deps": {
          "upstream": [
            "Python",
            "特征工程",
            "数据仓库",
            "大数据平台",
            "实时数据",
            "SQL"
          ],
          "downstream": [
            "BI"
          ]
        },
        "source": "《统计学习方法》；scikit-learn 文档"
      },
      "l3": {
        "tools": [
          "scikit-learn",
          "XGBoost/LightGBM",
          "PyTorch",
          "MLflow",
          "Feast"
        ],
        "code": "from sklearn.ensemble import GradientBoostingClassifier\nfrom sklearn.model_selection import train_test_split\nX_train,X_test,y_train,y_test=train_test_split(X,y,test_size=0.2,shuffle=False)\nclf=GradientBoostingClassifier().fit(X_train,y_train)\nprint(clf.score(X_test,y_test))",
        "resources": [
          {
            "name": "《统计学习方法》",
            "note": "李航，清华大学出版社"
          },
          {
            "name": "scikit-learn",
            "note": "官方指南"
          }
        ],
        "source": "清华大学出版社；scikit-learn Docs"
      }
    }
  },
  {
    "id": "feature",
    "name": "特征工程",
    "category": "intel",
    "detail": {
      "subtitle": "把原始数据变成模型可消费的信号",
      "l1": {
        "definition": "特征工程从原始日志/仓表构造、选择与服务化模型输入特征，含离线批量与在线点查。",
        "why": "模型上限常由特征决定；需要与实时、仓、质量体系协同。",
        "source": "《特征工程入门与实践》；Feast 文档；广告/推荐特征平台实践"
      },
      "l2": {
        "principles": [
          "训练集与服务一致性（训练-服务偏差）",
          "时间旅行：只用当时可知信息",
          "特征存储与监控漂移"
        ],
        "terms": [
          "特征存储 Feature Store",
          "点查/批量",
          "嵌入 Embedding",
          "交叉特征",
          "训练-服务偏差"
        ],
        "topics": [
          {
            "name": "构造方法",
            "text": "统计（次数/频次）、时间窗聚合、交叉、文本/图像嵌入。",
            "children": [
              {
                "name": "窗口特征",
                "text": "用户近 1h/1d/7d 行为计数。",
                "methods": [
                  "SQL 窗口/Flink 窗口",
                  "避免泄露：截止事件时间"
                ]
              },
              {
                "name": "编码",
                "text": "One-Hot、Target Encoding（慎防泄露）、Hashing。"
              }
            ]
          },
          {
            "name": "Feature Store",
            "text": "注册、版本、离线物化、在线 KV 服务。",
            "children": [
              {
                "name": "Feast 等",
                "text": "定义 Entity/FeatureView；物化到 Redis/仓。"
              }
            ]
          },
          {
            "name": "质量与监控",
            "text": "空值率、分布漂移 PSI、服务延迟。"
          },
          {
            "name": "与指标层关系",
            "text": "部分特征可从原子指标派生，但不等于报表指标。"
          }
        ],
        "deps": {
          "upstream": [
            "Python",
            "数据仓库",
            "实时数据",
            "数据湖",
            "指标/语义层",
            "数据质量"
          ],
          "downstream": [
            "机器学习/算法"
          ]
        },
        "source": "Feast Docs；O'Reilly 特征工程相关；业界特征平台分享"
      },
      "l3": {
        "tools": [
          "Feast",
          "Tecton",
          "Redis（在线）",
          "Spark/Flink（离线/近线）",
          "pandas"
        ],
        "code": "# 伪代码：近7日购买次数特征\n# SELECT user_id, COUNT(*) AS buy_7d\n# FROM events WHERE type='buy' AND ts < label_time AND ts >= label_time-7d\n# GROUP BY user_id",
        "resources": [
          {
            "name": "Feast Documentation",
            "note": "官方"
          },
          {
            "name": "《特征工程入门与实践》",
            "note": "相关译著/教材"
          }
        ],
        "source": "Feast；特征工程专著"
      }
    }
  },
  {
    "id": "lineage",
    "name": "数据血缘",
    "category": "govern",
    "detail": {
      "subtitle": "数据从哪来到哪去 · 影响分析与审计",
      "l1": {
        "definition": "数据血缘记录字段/表/任务级的来源与去向，支持影响分析、问题追溯与合规。",
        "why": "改一张表前能看见下游报表/模型；事故时可快速定位上游。",
        "source": "W3C PROV；OpenLineage；DataHub/Atlas"
      },
      "l2": {
        "principles": [
          "自动采集优于手工维护",
          "粒度：表级→字段级→任务运行级",
          "与调度/质检元数据打通"
        ],
        "terms": [
          "OpenLineage",
          "PROV-O",
          "影响分析",
          "字段级血缘",
          "元数据仓库"
        ],
        "topics": [
          {
            "name": "采集方式",
            "text": "解析 SQL、监听 Spark/Flink OpenLineage、调度钩子。",
            "children": [
              {
                "name": "OpenLineage",
                "text": "标准化 Job/Dataset facet。"
              }
            ]
          },
          {
            "name": "应用场景",
            "text": "变更影响、故障溯源、合规审计、成本归集。"
          },
          {
            "name": "可视化",
            "text": "图导航；与目录、治理平台同屏。"
          },
          {
            "name": "挑战",
            "text": "动态 SQL、临时表、BI 抽取导致断链。"
          }
        ],
        "deps": {
          "upstream": [
            "ETL",
            "调度编排",
            "大数据平台"
          ],
          "downstream": [
            "数据治理",
            "数据质量",
            "BI"
          ]
        },
        "source": "W3C PROV-O；OpenLineage Spec；DataHub"
      },
      "l3": {
        "tools": [
          "OpenLineage",
          "DataHub",
          "Apache Atlas",
          "Amundsen",
          "自研解析器"
        ],
        "code": "// OpenLineage 事件概念：run + job + inputs/outputs datasets",
        "resources": [
          {
            "name": "OpenLineage",
            "note": "https://openlineage.io/"
          },
          {
            "name": "W3C PROV",
            "note": "溯源模型"
          }
        ],
        "source": "OpenLineage；W3C PROV"
      }
    }
  },
  {
    "id": "governance",
    "name": "数据治理",
    "category": "govern",
    "detail": {
      "subtitle": "政策、角色与流程 · 让数据可用、可信、安全",
      "l1": {
        "definition": "数据治理是组织内关于数据资产的决策权、标准、质量、安全与生命周期的制度与工具集合。",
        "why": "技术栈再强，没有治理也会口径混乱与合规风险。",
        "source": "DAMA-DMBOK；国家标准数据管理相关；《数据治理》实践书"
      },
      "l2": {
        "principles": [
          "治理是产品+组织，不只是工具",
          "先主数据与指标标准，再铺开",
          "安全与可用平衡"
        ],
        "terms": [
          "数据责任人 Steward",
          "主数据 MDM",
          "数据分类分级",
          "生命周期",
          "合规"
        ],
        "topics": [
          {
            "name": "组织与角色",
            "text": "Owner/Steward/Consumer；委员会决策。"
          },
          {
            "name": "标准与主数据",
            "text": "客户/商品等实体唯一标识与权威源。"
          },
          {
            "name": "安全合规",
            "text": "分类分级、脱敏、权限审批、审计日志。",
            "children": [
              {
                "name": "隐私",
                "text": "最小化采集；用途限定。"
              }
            ]
          },
          {
            "name": "生命周期",
            "text": "创建→使用→归档→销毁；湖上冷热分层。"
          },
          {
            "name": "工具落地",
            "text": "目录、质量、血缘、权限中心协同。"
          }
        ],
        "deps": {
          "upstream": [
            "数据血缘",
            "数据质量",
            "数据仓库"
          ],
          "downstream": [
            "BI",
            "指标/语义层",
            "ETL"
          ]
        },
        "source": "DAMA-DMBOK；相关国标与实践"
      },
      "l3": {
        "tools": [
          "DataHub/Collibra",
          "Ranger/统一权限",
          "主数据系统",
          "政策知识库"
        ],
        "code": "-- 策略示例：PII 列默认脱敏视图；申请制访问生产明细",
        "resources": [
          {
            "name": "DAMA-DMBOK",
            "note": "数据管理知识体系"
          },
          {
            "name": "DataHub",
            "note": "开源治理平台"
          }
        ],
        "source": "DAMA；开源治理工具文档"
      }
    }
  },
  {
    "id": "dq",
    "name": "数据质量",
    "category": "govern",
    "detail": {
      "subtitle": "完整性、准确性、一致性、及时性的度量与管控",
      "l1": {
        "definition": "数据质量通过规则、度量与闭环工单，保证数据满足约定用途的可信度。",
        "why": "质量差的数据会污染报表、特征与治理信誉；要「可测、可告警、可归因」。",
        "source": "DAMA 质量维度；Great Expectations；《数据质量实践》相关"
      },
      "l2": {
        "principles": [
          "质量维度可度量",
          "规则即代码，进调度",
          "事故要归因到表/字段/任务"
        ],
        "terms": [
          "完整性",
          "准确性",
          "一致性",
          "及时性 Timeliness",
          "唯一性",
          "GE / Soda"
        ],
        "topics": [
          {
            "name": "质量维度",
            "text": "完整、准确、一致、及时、唯一、有效。",
            "children": [
              {
                "name": "完整性",
                "text": "非空、必需字段覆盖率。"
              },
              {
                "name": "一致性",
                "text": "跨系统同一实体对齐；枚举合法。"
              },
              {
                "name": "及时性",
                "text": "到达 SLA；水位延迟。"
              }
            ]
          },
          {
            "name": "规则实现",
            "text": "SQL assert、GE expectation、仓内约束。",
            "children": [
              {
                "name": "示例规则",
                "text": "主键唯一、外键孤儿率、金额>=0、日环比波动。",
                "methods": [
                  "unique",
                  "not_null",
                  "row_count 波动",
                  "分布上下界"
                ]
              }
            ]
          },
          {
            "name": "闭环",
            "text": "告警→工单→修复→回归；与血缘定位上游。"
          },
          {
            "name": "嵌入流水线",
            "text": "关键 DAG 节点后强制质检门禁。"
          }
        ],
        "deps": {
          "upstream": [
            "ETL",
            "调度编排",
            "数据血缘"
          ],
          "downstream": [
            "数据治理",
            "BI",
            "特征工程",
            "指标/语义层"
          ]
        },
        "source": "DAMA-DMBOK Quality；Great Expectations Docs；Soda"
      },
      "l3": {
        "tools": [
          "Great Expectations",
          "Soda",
          "dbt tests",
          "自定义 SQL 监控"
        ],
        "code": "# Great Expectations 概念\n# expect_column_values_to_not_be_null\n# expect_column_values_to_be_unique\n# expect_table_row_count_to_be_between",
        "resources": [
          {
            "name": "Great Expectations",
            "note": "官方文档"
          },
          {
            "name": "dbt tests",
            "note": "schema.yml"
          }
        ],
        "source": "GX Docs；dbt；DAMA"
      }
    }
  }
];
    const LAYOUT = {
  "database": {
    "col": 0,
    "row": 0.5
  },
  "sql": {
    "col": 1,
    "row": 0
  },
  "python": {
    "col": 1,
    "row": 1
  },
  "etl": {
    "col": 2,
    "row": 0
  },
  "realtime": {
    "col": 2,
    "row": 0.55
  },
  "orchestrate": {
    "col": 2,
    "row": 1.1
  },
  "bigdata": {
    "col": 3,
    "row": 0
  },
  "dwh": {
    "col": 3,
    "row": 0.55
  },
  "lake": {
    "col": 3,
    "row": 1.1
  },
  "modeling": {
    "col": 4,
    "row": 0
  },
  "bi": {
    "col": 4,
    "row": 0.55
  },
  "metrics": {
    "col": 4,
    "row": 1.1
  },
  "ml": {
    "col": 5,
    "row": 0.25
  },
  "feature": {
    "col": 5,
    "row": 0.9
  },
  "lineage": {
    "col": 2.0,
    "row": 2
  },
  "governance": {
    "col": 3.2,
    "row": 2
  },
  "dq": {
    "col": 4.4,
    "row": 2
  }
};
    const links = [
  {
    "source": "database",
    "target": "sql",
    "trunk": true
  },
  {
    "source": "database",
    "target": "etl",
    "trunk": true
  },
  {
    "source": "database",
    "target": "realtime",
    "trunk": true
  },
  {
    "source": "sql",
    "target": "etl",
    "trunk": true
  },
  {
    "source": "python",
    "target": "etl",
    "trunk": true
  },
  {
    "source": "python",
    "target": "orchestrate",
    "trunk": true
  },
  {
    "source": "realtime",
    "target": "etl",
    "trunk": true
  },
  {
    "source": "orchestrate",
    "target": "etl",
    "trunk": true
  },
  {
    "source": "etl",
    "target": "bigdata",
    "trunk": true
  },
  {
    "source": "etl",
    "target": "dwh",
    "trunk": true
  },
  {
    "source": "etl",
    "target": "lake",
    "trunk": true
  },
  {
    "source": "bigdata",
    "target": "dwh",
    "trunk": true
  },
  {
    "source": "lake",
    "target": "dwh",
    "trunk": true
  },
  {
    "source": "dwh",
    "target": "modeling",
    "trunk": true
  },
  {
    "source": "modeling",
    "target": "bi",
    "trunk": true
  },
  {
    "source": "dwh",
    "target": "metrics",
    "trunk": true
  },
  {
    "source": "metrics",
    "target": "bi",
    "trunk": true
  },
  {
    "source": "dwh",
    "target": "ml",
    "trunk": true
  },
  {
    "source": "python",
    "target": "feature",
    "trunk": true
  },
  {
    "source": "feature",
    "target": "ml",
    "trunk": true
  },
  {
    "source": "etl",
    "target": "lineage",
    "trunk": true
  },
  {
    "source": "lineage",
    "target": "governance",
    "trunk": true
  },
  {
    "source": "dq",
    "target": "governance",
    "trunk": true
  },
  {
    "source": "orchestrate",
    "target": "dq",
    "trunk": true
  },
  {
    "source": "database",
    "target": "modeling",
    "trunk": false
  },
  {
    "source": "sql",
    "target": "bi",
    "trunk": false
  },
  {
    "source": "sql",
    "target": "metrics",
    "trunk": false
  },
  {
    "source": "sql",
    "target": "ml",
    "trunk": false
  },
  {
    "source": "realtime",
    "target": "lake",
    "trunk": false
  },
  {
    "source": "realtime",
    "target": "feature",
    "trunk": false
  },
  {
    "source": "realtime",
    "target": "bigdata",
    "trunk": false
  },
  {
    "source": "lake",
    "target": "feature",
    "trunk": false
  },
  {
    "source": "lake",
    "target": "ml",
    "trunk": false
  },
  {
    "source": "bigdata",
    "target": "ml",
    "trunk": false
  },
  {
    "source": "bigdata",
    "target": "python",
    "trunk": false
  },
  {
    "source": "metrics",
    "target": "feature",
    "trunk": false
  },
  {
    "source": "modeling",
    "target": "etl",
    "trunk": false
  },
  {
    "source": "governance",
    "target": "bi",
    "trunk": false
  },
  {
    "source": "governance",
    "target": "etl",
    "trunk": false
  },
  {
    "source": "dq",
    "target": "etl",
    "trunk": false
  },
  {
    "source": "lineage",
    "target": "dq",
    "trunk": false
  },
  {
    "source": "ml",
    "target": "bi",
    "trunk": false
  }
];

    const legendEl = document.getElementById("legend");
    Object.entries(CATEGORIES).forEach(([key, cat]) => {
      const item = document.createElement("div");
      item.className = "legend-item";
      item.innerHTML = `<span class="legend-dot" style="background:${cat.color};color:${cat.color}"></span>${(cat.label.split(" ")[1] || cat.label)}`;
      legendEl.appendChild(item);
    });

    const neighbors = {};
    nodes.forEach(n => { neighbors[n.id] = new Set(); });
    links.forEach(l => {
      neighbors[l.source].add(l.target);
      neighbors[l.target].add(l.source);
    });

    const panel = document.getElementById("panel");
    const panelCat = document.getElementById("panelCat");
    const panelTitle = document.getElementById("panelTitle");
    const panelSub = document.getElementById("panelSub");
    const panelBody = document.getElementById("panelBody");
    const btnClose = document.getElementById("btnClose");
    let selectedId = null;
    let panelState = { mode: "overview", engineId: null, compareOn: false, compareIds: [] };

    function escapeHtml(str) {
      return String(str)
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;");
    }

    function renderDeepNode(node) {
      const methods = (node.methods && node.methods.length)
        ? `<ul class="method-list">${node.methods.map(m => {
            const parts = String(m).split(" — ");
            if (parts.length > 1) {
              return `<li><strong>${escapeHtml(parts[0])}</strong> — ${escapeHtml(parts.slice(1).join(" — "))}</li>`;
            }
            return `<li>${escapeHtml(m)}</li>`;
          }).join("")}</ul>`
        : "";
      const code = node.code ? `<div class="code-block">${escapeHtml(node.code)}</div>` : "";
      const kids = (node.children && node.children.length)
        ? node.children.map(renderDeepNode).join("")
        : "";
      return `<details>
          <summary>${escapeHtml(node.name)}</summary>
          <div class="deep-body">
            ${node.text ? `<p>${escapeHtml(node.text)}</p>` : ""}
            ${methods}${code}${kids}
          </div>
        </details>`;
    }

    function renderTopics(topics) {
      if (!topics || !topics.length) return "";
      const tree = topics.map(t => {
        if (!t.children && !t.methods && !t.code) {
          return `<div class="topic-block"><div class="topic-name">${escapeHtml(t.name)}</div><p>${escapeHtml(t.text || "")}</p></div>`;
        }
        return renderDeepNode(t);
      }).join("");
      return `<h4>知识细分（可逐层展开）</h4>
        <p class="deep-hint">// 点击 + 下钻：主题 → 子题 → 方法 → 示例</p>
        <div class="deep-tree">${tree}</div>`;
    }

    function renderLayers(d, openL3) {
      const depUp = (d.l2.deps.upstream || []).map(n => `<span class="dep-tag">${escapeHtml(n)}</span>`).join("")
        || "<span class='source' style='border:none;padding:0;margin:0'>// 源头模块</span>";
      const depDown = (d.l2.deps.downstream || []).map(n => `<span class="dep-tag">${escapeHtml(n)}</span>`).join("") || "—";
      const resources = (d.l3.resources || []).map(r =>
        `<li><span class="resource-link">${escapeHtml(r.name)}</span> — ${escapeHtml(r.note)}</li>`).join("");
      return `
        <div class="layer open" data-layer="l1">
          <button class="layer-toggle" type="button">
            <span class="layer-level">L1</span><span class="layer-title">概念 CONCEPT</span><span class="layer-chevron">▼</span>
          </button>
          <div class="layer-content">
            <h4>一句话定义</h4><p>${escapeHtml(d.l1.definition)}</p>
            <h4>为什么重要</h4><p>${escapeHtml(d.l1.why)}</p>
            <span class="source">${escapeHtml(d.l1.source)}</span>
          </div>
        </div>
        <div class="layer open" data-layer="l2">
          <button class="layer-toggle" type="button">
            <span class="layer-level">L2</span><span class="layer-title">理解 UNDERSTAND</span><span class="layer-chevron">▼</span>
          </button>
          <div class="layer-content">
            <h4>核心原理</h4><ul>${d.l2.principles.map(p => `<li>${escapeHtml(p)}</li>`).join("")}</ul>
            <h4>关键术语</h4><ul>${d.l2.terms.map(t => `<li>${escapeHtml(t)}</li>`).join("")}</ul>
            ${renderTopics(d.l2.topics)}
            <h4>上游依赖</h4><div class="dep-tags">${depUp}</div>
            <h4>下游影响</h4><div class="dep-tags">${depDown}</div>
            <span class="source">${escapeHtml(d.l2.source)}</span>
          </div>
        </div>
        <div class="layer${openL3 ? " open" : ""}" data-layer="l3">
          <button class="layer-toggle" type="button">
            <span class="layer-level">L3</span><span class="layer-title">实操 PRACTICE</span><span class="layer-chevron">▼</span>
          </button>
          <div class="layer-content">
            <h4>常用工具</h4><ul>${d.l3.tools.map(t => `<li>${escapeHtml(t)}</li>`).join("")}</ul>
            <h4>代码示例</h4><div class="code-block">${escapeHtml(d.l3.code)}</div>
            <h4>学习资源</h4><ul>${resources}</ul>
            <span class="source">${escapeHtml(d.l3.source)}</span>
          </div>
        </div>`;
    }

    function bindLayerToggles() {
      panelBody.querySelectorAll(".layer-toggle").forEach(btn => {
        btn.addEventListener("click", () => btn.parentElement.classList.toggle("open"));
      });
    }

    function renderCompareTable(engines, ids) {
      const cols = engines.filter(e => ids.includes(e.id));
      if (cols.length < 2) return `<p class="deep-hint">// 请勾选 2～3 个引擎生成对比表</p>`;
      const head = `<tr><th>维度</th>${cols.map(c => `<th>${escapeHtml(c.name)}</th>`).join("")}</tr>`;
      const body = COMPARE_FIELDS.map(f => {
        const tds = cols.map(c => `<td>${escapeHtml((c.compare && c.compare[f.key]) || "—")}</td>`).join("");
        return `<tr><td class="row-label">${escapeHtml(f.label)}</td>${tds}</tr>`;
      }).join("");
      return `<div class="compare-table-wrap"><table class="compare-table"><thead>${head}</thead><tbody>${body}</tbody></table></div>`;
    }

    function renderCatalogPanel(node) {
      const engines = node.engines || [];
      const label = node.catalogLabel || "列表";

      if (panelState.mode === "engine" && panelState.engineId) {
        const eng = engines.find(e => e.id === panelState.engineId);
        if (!eng) { panelState.mode = "list"; return renderCatalogPanel(node); }
        panelTitle.textContent = eng.name;
        panelSub.textContent = eng.tagline || node.detail.subtitle;
        panelBody.innerHTML = `
          <div class="crumb"><a data-act="back-list">← ${escapeHtml(label)}</a> / ${escapeHtml(eng.name)}</div>
          ${renderLayers(eng.detail, false)}`;
        bindLayerToggles();
        panelBody.querySelector("[data-act=back-list]").addEventListener("click", () => {
          panelState.mode = "list"; panelState.engineId = null; renderPanel(node);
        });
        return;
      }

      panelTitle.textContent = node.name;
      panelSub.textContent = node.detail.subtitle;

      if (panelState.mode === "overview") {
        panelBody.innerHTML = `
          <div class="mode-tabs">
            <button type="button" class="active" data-mode="overview">概览</button>
            <button type="button" data-mode="list">${escapeHtml(label)}</button>
          </div>
          ${renderLayers(node.detail, false)}`;
        bindLayerToggles();
        panelBody.querySelectorAll(".mode-tabs button").forEach(b => {
          b.addEventListener("click", () => {
            panelState.mode = b.getAttribute("data-mode");
            panelState.compareOn = false;
            panelState.compareIds = [];
            renderPanel(node);
          });
        });
        return;
      }

      const compareClass = panelState.compareOn ? " compare-mode" : "";
      const cards = engines.map(e => {
        const checked = panelState.compareIds.includes(e.id);
        return `<div class="engine-card${checked ? " selected" : ""}" data-eid="${escapeHtml(e.id)}">
          <div class="ename">${escapeHtml(e.name)}</div>
          <div class="etag">${escapeHtml(e.tagline || "")}</div>
          <div class="echeck">${checked ? "✓ 已选对比" : "点击勾选对比"}</div>
        </div>`;
      }).join("");

      panelBody.innerHTML = `
        <div class="mode-tabs">
          <button type="button" data-mode="overview">概览</button>
          <button type="button" class="active" data-mode="list">${escapeHtml(label)}</button>
          <button type="button" data-mode="compare" class="${panelState.compareOn ? "active" : ""}">对比</button>
        </div>
        <div class="compare-bar">
          <span>${panelState.compareOn ? "对比模式：勾选 2～3 个引擎" : "点卡片查看详情；或切换「对比」"} · 已选 ${panelState.compareIds.length}</span>
          <button type="button" id="btnDoCompare" ${(!panelState.compareOn || panelState.compareIds.length < 2 || panelState.compareIds.length > 3) ? "disabled" : ""}>生成对比表</button>
        </div>
        <div class="engine-grid${compareClass}" id="engineGrid">${cards}</div>
        <div id="compareResult"></div>`;

      panelBody.querySelectorAll(".mode-tabs button").forEach(b => {
        b.addEventListener("click", () => {
          const m = b.getAttribute("data-mode");
          if (m === "compare") {
            panelState.mode = "list";
            panelState.compareOn = true;
          } else {
            panelState.mode = m;
            panelState.compareOn = false;
            panelState.compareIds = [];
          }
          renderPanel(node);
        });
      });

      panelBody.querySelectorAll(".engine-card").forEach(card => {
        card.addEventListener("click", () => {
          const eid = card.getAttribute("data-eid");
          if (panelState.compareOn) {
            const idx = panelState.compareIds.indexOf(eid);
            if (idx >= 0) panelState.compareIds.splice(idx, 1);
            else if (panelState.compareIds.length < 3) panelState.compareIds.push(eid);
            renderPanel(node);
          } else {
            panelState.mode = "engine";
            panelState.engineId = eid;
            renderPanel(node);
          }
        });
      });

      const btn = panelBody.querySelector("#btnDoCompare");
      if (btn) {
        btn.addEventListener("click", () => {
          panelBody.querySelector("#compareResult").innerHTML =
            `<h4>差异对比</h4>` + renderCompareTable(engines, panelState.compareIds);
        });
      }
    }

    function renderPanel(node) {
      const cat = CATEGORIES[node.category];
      panelCat.textContent = cat.label;
      panelCat.style.background = cat.color;

      if (node.catalog && node.engines && node.engines.length) {
        renderCatalogPanel(node);
        panel.classList.add("open");
        return;
      }

      panelTitle.textContent = node.name;
      panelSub.textContent = node.detail.subtitle;
      panelBody.innerHTML = renderLayers(node.detail, false);
      bindLayerToggles();
      panel.classList.add("open");
    }

    function closePanel() {
      selectedId = null;
      panel.classList.remove("open");
      panelState = { mode: "overview", engineId: null, compareOn: false, compareIds: [] };
      resetHighlight();
    }
    btnClose.addEventListener("click", closePanel);

    const graphEl = document.getElementById("graph");
    const width = () => graphEl.clientWidth;
    const height = () => graphEl.clientHeight;
    let showAllLinks = false;

    const svg = d3.select("#graph").append("svg");
    const defs = svg.append("defs");
    const glow = defs.append("filter").attr("id", "node-glow");
    glow.append("feGaussianBlur").attr("stdDeviation", "2.5").attr("result", "coloredBlur");
    const feMerge = glow.append("feMerge");
    feMerge.append("feMergeNode").attr("in", "coloredBlur");
    feMerge.append("feMergeNode").attr("in", "SourceGraphic");

    defs.append("marker").attr("id", "arrow").attr("viewBox", "0 -4 8 8")
      .attr("refX", 10).attr("refY", 0).attr("markerWidth", 6).attr("markerHeight", 6).attr("orient", "auto")
      .append("path").attr("d", "M0,-4L8,0L0,4").attr("fill", "#475569");
    defs.append("marker").attr("id", "arrow-active").attr("viewBox", "0 -4 8 8")
      .attr("refX", 10).attr("refY", 0).attr("markerWidth", 7).attr("markerHeight", 7).attr("orient", "auto")
      .append("path").attr("d", "M0,-4L8,0L0,4").attr("fill", "#22d3ee");

    const gRoot = svg.append("g");
    const zoom = d3.zoom().scaleExtent([0.4, 2.2]).on("zoom", (event) => {
      gRoot.attr("transform", event.transform);
    });
    svg.call(zoom);

    const stageG = gRoot.append("g").attr("class", "stages");
    const linkG = gRoot.append("g").attr("class", "links");
    const nodeG = gRoot.append("g").attr("class", "nodes");
    const laneG = gRoot.append("g").attr("class", "lanes");

    function layoutMetrics() {
      const w = Math.max(width(), 980);
      const h = Math.max(height(), 600);
      const padX = 60, padTop = 58, padBottom = 28, govBand = 110;
      const mainH = h - padTop - padBottom - govBand;
      const colW = (w - padX * 2) / STAGES.length;
      return { w, h, padX, padTop, padBottom, govBand, mainH, colW };
    }

    function placeNodes() {
      const m = layoutMetrics();
      nodes.forEach(n => {
        const L = LAYOUT[n.id] || { col: 0, row: 0 };
        const x = m.padX + (L.col + 0.5) * m.colW;
        let y;
        if (L.row >= 2) {
          y = m.padTop + m.mainH + m.govBand * 0.55;
        } else if (L.row === 0.5) {
          y = m.padTop + m.mainH * 0.48;
        } else {
          const t = Math.min(1.15, Math.max(0, L.row)) / 1.15;
          y = m.padTop + m.mainH * (0.22 + t * 0.56);
        }
        n.x = x; n.y = y; n.fx = x; n.fy = y;
      });
      return m;
    }

    function drawStages(m) {
      stageG.selectAll("*").remove();
      laneG.selectAll("*").remove();
      STAGES.forEach((s, i) => {
        const x = m.padX + i * m.colW;
        stageG.append("rect").attr("class", "stage-band")
          .attr("x", x + 6).attr("y", m.padTop - 28)
          .attr("width", m.colW - 12).attr("height", m.mainH + 20).attr("rx", 10);
        stageG.append("text").attr("class", "stage-label")
          .attr("x", x + m.colW / 2).attr("y", m.padTop - 10).text(s.label);
      });
      laneG.append("rect").attr("class", "stage-band")
        .attr("x", m.padX + m.colW * 1.6).attr("y", m.padTop + m.mainH + 8)
        .attr("width", m.colW * 3.2).attr("height", m.govBand - 16).attr("rx", 10)
        .attr("fill", "rgba(167, 139, 250, 0.06)").attr("stroke", "rgba(167, 139, 250, 0.2)");
      laneG.append("text").attr("class", "lane-label")
        .attr("x", m.padX + m.colW * 3.2).attr("y", m.padTop + m.mainH + 28)
        .attr("text-anchor", "middle").attr("fill", "#a78bfa")
        .text("GOVERNANCE  治理横贯层（血缘 · 治理 · 质量）");
    }

    function linkPath(d) {
      const sx = d.source.x, sy = d.source.y, tx = d.target.x, ty = d.target.y;
      const dx = tx - sx, dy = ty - sy;
      const dist = Math.hypot(dx, dy) || 1;
      const pad = 32;
      const x2 = tx - (dx / dist) * pad, y2 = ty - (dy / dist) * pad;
      const mx = (sx + x2) / 2, my = (sy + y2) / 2;
      const nx = -dy / dist, ny = dx / dist;
      const bend = Math.min(36, dist * 0.12) * (d.trunk ? 0.35 : 0.9);
      return `M${sx},${sy}Q${mx + nx * bend},${my + ny * bend} ${x2},${y2}`;
    }

    placeNodes();
    drawStages(layoutMetrics());

    const link = linkG.selectAll("path").data(links).join("path")
      .attr("class", d => d.trunk ? "link" : "link secondary")
      .attr("marker-end", "url(#arrow)");

    const nodeById = Object.fromEntries(nodes.map(n => [n.id, n]));
    links.forEach(l => { l.source = nodeById[l.source]; l.target = nodeById[l.target]; });

    const node = nodeG.selectAll("g").data(nodes).join("g")
      .attr("class", "node")
      .attr("transform", d => `translate(${d.x},${d.y})`)
      .call(d3.drag()
        .on("start", (event) => event.sourceEvent.stopPropagation())
        .on("drag", (event, d) => { d.x = d.fx = event.x; d.y = d.fy = event.y; tick(); }));

    node.append("circle").attr("r", 28)
      .attr("fill", d => CATEGORIES[d.category].color)
      .attr("filter", "url(#node-glow)");
    node.append("text").text(d => shortLabel(d.name));
    node.append("text").attr("class", "sublabel").attr("dy", 40)
      .text(d => d.catalog ? "CATALOG" : (d.name.length > 5 ? d.name : ""));

    node.on("click", (event, d) => {
      event.stopPropagation();
      panelState = { mode: d.catalog ? "list" : "overview", engineId: null, compareOn: false, compareIds: [] };
      selectNode(d);
    });
    svg.on("click", () => closePanel());

    function tick() {
      link.attr("d", linkPath);
      node.attr("transform", d => `translate(${d.x},${d.y})`);
      applyLinkVisibility();
    }

    function applyLinkVisibility() {
      link.classed("hidden", d => {
        if (selectedId) return false;
        return !showAllLinks && !d.trunk;
      });
      if (!selectedId) {
        link.classed("active", false).classed("dimmed", false)
          .attr("marker-end", d => (!showAllLinks && !d.trunk) ? null : "url(#arrow)");
      }
    }

    function shortLabel(name) {
      const map = {
        "实时数据": "实时", "机器学习/算法": "ML", "大数据平台": "大数据",
        "数据仓库": "数仓", "数据建模": "建模", "数据治理": "治理",
        "数据血缘": "血缘", "调度编排": "调度", "指标/语义层": "指标",
        "数据质量": "质量", "特征工程": "特征", "数据湖": "数据湖", "数据库": "数据库"
      };
      return map[name] || name;
    }

    function selectNode(d) {
      selectedId = d.id;
      const related = neighbors[d.id];
      node.classed("selected", n => n.id === d.id)
        .classed("related", n => related.has(n.id))
        .classed("dimmed", n => n.id !== d.id && !related.has(n.id));
      link.classed("hidden", false)
        .classed("active", l => l.source.id === d.id || l.target.id === d.id)
        .classed("dimmed", l => l.source.id !== d.id && l.target.id !== d.id)
        .attr("marker-end", l =>
          (l.source.id === d.id || l.target.id === d.id) ? "url(#arrow-active)" : "url(#arrow)");
      renderPanel(d);
    }

    function resetHighlight() {
      selectedId = null;
      node.classed("selected", false).classed("related", false).classed("dimmed", false);
      applyLinkVisibility();
      tick();
    }

    document.getElementById("btnTrunk").addEventListener("click", () => {
      showAllLinks = false;
      document.getElementById("btnTrunk").classList.add("active");
      document.getElementById("btnAll").classList.remove("active");
      if (!selectedId) applyLinkVisibility();
    });
    document.getElementById("btnAll").addEventListener("click", () => {
      showAllLinks = true;
      document.getElementById("btnAll").classList.add("active");
      document.getElementById("btnTrunk").classList.remove("active");
      if (!selectedId) applyLinkVisibility();
    });

    function relayout() {
      const m = placeNodes();
      drawStages(m);
      tick();
      const scale = Math.min(1, width() / m.w, height() / m.h);
      const tx = (width() - m.w * scale) / 2;
      const ty = (height() - m.h * scale) / 2;
      svg.call(zoom.transform, d3.zoomIdentity.translate(Math.max(0, tx), Math.max(0, ty)).scale(scale < 1 ? scale : 1));
    }

    window.addEventListener("resize", () => relayout());
    tick();
    relayout();

  